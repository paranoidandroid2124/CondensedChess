...
// staffPicks removed - cms module deleted
// def staffPicks(p: lila.cms.CmsPage.Render)(using Context) = ...

// streamers removed - streamer module deleted
def streamers(streamers: List[UserId])(using Translate) =
  emptyFrag  // streamer.bits.contextual removed
...
def show(
  ...
  Page(s.name.value)
    ...
    .graph(
      ...
      frag(
        main(cls := "analyse")
        // streamer.bits.contextual removed
      )
