...
final class CommentaryController(
  ...
  private def normalizeProbeResultsByPly(
      entries: Option[List[lila.commentary.ProbeResultsByPlyEntry]]
  ): Map[Int, List[lila.commentary.model.ProbeResult]] =
    entries
      .getOrElse(Nil)
      .collect {
  ...
  private val anonFullGameDailyLimiter =
    env.memo.mongoRateLimitApi[String](
      name = "commentary.full.anon.ip.daily",
      credits = 1,
      duration = 1.day
    )
  ...
  private val freeFullGameDailyLimiter =
    env.memo.mongoRateLimitApi[String](
      name = "commentary.full.free.user.daily",
      credits = 1,
      duration = 1.day
    )
  ...
  private val premiumFullGameRollingLimiter =
    env.memo.mongoRateLimitApi[String](
      name = "commentary.full.premium.user.30d",
      credits = 900,
      duration = 30.day
    )
  ...
  private val premiumBurstLimiter =
    env.memo.mongoRateLimitApi[String](
      name = "commentary.request.premium.user.burst",
      credits = 60,
      duration = 1.minute
    )
      ...
      analysisReq =>
        validatedGameChronicleRequest(analysisReq).fold(
          ...
          validReq =>
            ...
            def runGameChronicle(allowAiPolish: Boolean) =
              api
                .analyzeGameChronicleLocal(
                  ...
                  commentaryMode = resolvedGameAnalysisCommentaryMode,
            ...
            if suppliedProbeResults && !wantsProbeRefinement then
              ...
              refinementToken match
                case Some(token)
                  ...
                  runGameChronicle(allowAiPolish = true)
                case _ =>
                  Forbidden(
                    Json.obj(
              ...
              withFullGameQuota:
                allowAiPolish =>
                  runGameChronicle(allowAiPolish)
        )
    )
        ...
        api.getGameChronicleAsyncStatus(id, statusToken.get) match
          case Some(status) =>
            ...
            val isCcaEnabled = ctx.me.map(_.userId.value)
              ...
              .exists(uid => new lila.commentary.DefeatDnaApi().isCcaEnabledForUser(uid))
  ...
  private def resolvedGameAnalysisCommentaryMode(using ctx: Context): String =
  ...
  private def gameChronicleEnvelope(
      response: lila.commentary.GameChronicleResponse,
      pgn: String
  )(using ctx: Context): JsObject =
    val uidOpt = ctx.me.map(_.userId.value)
    val isCcaEnabled = uidOpt.exists(uid => new lila.commentary.DefeatDnaApi().isCcaEnabledForUser(uid))
    val sanitizedResponse = lila.commentary.UserFacingPayloadSanitizer.sanitize(response)
  ...
  private def withFullGameQuota(op: Boolean => Fu[Result])(using ctx: Context): Fu[Result] =
    if hasPremiumExperience then
      ...
      withHardQuota(
        ...
        key = ipRequesterKey,
        msg = "commentary.full.anon.daily",
