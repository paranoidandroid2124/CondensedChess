...
const siteI18n = ((window as any).i18n?.site ?? {}) as Record<string, any>;
const t = (key: string, fallback: string): string => String(siteI18n[key] ?? fallback);
const tNbSeconds = (seconds: number): string =>
  typeof siteI18n.nbSeconds === 'function' ? String(siteI18n.nbSeconds(seconds)) : `${seconds}s`;
...
const addGameDuration = (el: HTMLCanvasElement, moveCentis: number[]) => {
  ...
  label.text(t('duration', 'Duration') + ' ' + formatClock(duration));
};
