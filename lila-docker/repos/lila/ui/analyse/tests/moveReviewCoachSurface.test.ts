...
describe('moveReview coach surface', () => {
  test('renders player-facing coach structure without developer labels', () => {
    ...
    assert.match(html, /On the board · Decision/);
    assert.match(html, /Move 1\/2/);
    assert.match(html, /Prev move/);
    assert.match(html, /Next move/);
    assert.match(html, /aria-label="Replay this line"/);
    ...
    assert.doesNotMatch(html, /Back to lesson board/);
  ...
  test('keeps board context rail synced with scene and move state', () => {
    ...
    assert.match(interactionHandlersSource, /On the board/);
    assert.doesNotMatch(interactionHandlersSource, /Board shows/);
    assert.doesNotMatch(interactionHandlersSource, /lesson step/);
