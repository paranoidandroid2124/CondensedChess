...
describe('frontend audit regressions', () => {
  ...
  test('strategic puzzle exposes live entry points and a non-404 exhausted state', () => {
    ...
    assert.match(strategicPuzzleTs, /sp-demo-panel sp-demo-panel--reveal is-open/);
    assert.match(strategicPuzzleTs, /sp-demo-lines sp-runtime-lines is-open/);
    assert.match(strategicPuzzleTs, /Give up and reveal the line/);
    assert.match(strategicPuzzleTs, /Reveal ends the attempt and opens the review below\./);
    assert.match(strategicPuzzleTs, /The review opened and this attempt counts as a give-up\./);
    ...
    assert.match(strategicPuzzleTs, /Move list stays hidden/);
    assert.match(strategicPuzzleTs, /data-action="hint"/);
