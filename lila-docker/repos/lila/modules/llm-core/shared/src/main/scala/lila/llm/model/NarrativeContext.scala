...
case class L1Snapshot(
  ...
  isKingExposed: Boolean = false // P14: Structured safety signal
)
...
enum ConfidenceLevel:
  case Engine     // Definitive - "Definite", backed by engine
  case Probe      // Verified - "Verified", backed by probe result
  case Heuristic  // Suggestive - "Pattern", board pattern heuristic
...
case class ErrorClassification(
  ...
  errorSummary: String         // "Tactics (300cp in 2 moves)" or "Positional"
)
