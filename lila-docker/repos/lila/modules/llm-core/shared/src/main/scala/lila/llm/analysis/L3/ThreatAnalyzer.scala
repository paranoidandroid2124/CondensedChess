...
object ThreatAnalyzer:
  ...
  private val MATERIAL_THREAT_THRESHOLD = lila.llm.analysis.Thresholds.URGENT_THREAT_CP     // 200cp
  private val URGENT_THREAT_THRESHOLD = lila.llm.analysis.Thresholds.CRITICAL_THREAT_CP      // 800cp
  private val IGNORABLE_THREAT_THRESHOLD = lila.llm.analysis.Thresholds.SIGNIFICANT_THREAT_CP // 100cp
  private val ONLY_DEFENSE_TOLERANCE = lila.llm.analysis.Thresholds.DEFENSE_TOLERANCE_CP        // 50cp
  private val MIN_DEPTH_FOR_RELIABILITY = lila.llm.analysis.Thresholds.MIN_RELIABLE_DEPTH  // 16
  ...
  private def extractOpponentThreats(motifs: List[Motif], isWhiteToMove: Boolean): List[Threat] =
    motifs.flatMap { motif =>
      ...
      if !isOpponentMotif then None
        ...
        val (baseLoss, turnsToImpact) = getBaseLoss(motifName)
        
        if baseLoss > 0 then
          val kind = 
            if isMateMotif(motifName) then ThreatKind.Mate
  ...
  // ============================================================
  // LOSS ESTIMATION (Consolidated from MotifLossTable)
  // ============================================================

  private def getBaseLoss(motifType: String): (Int, Int) = motifType.toLowerCase match
  ...
  private def isMateMotif(motifType: String): Boolean =
