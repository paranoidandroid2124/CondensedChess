  ...
  private def buildL1Snapshot(ctx: IntegratedContext): L1Snapshot = {
    ctx.features match {
      case Some(f) =>
        ...
        val isKingExposed = ourAttackers >= 2 || ourEscapes <= 1
    ...
    data.counterfactual.flatMap { cf =>
      ...
      if (cf.cpLoss < 50) None
        ...
        val errorSummary = if (isTactical) {
          s"Tactics(${cf.cpLoss}cp${if (missedMotifNames.nonEmpty) s", ${missedMotifNames.head}" else ""})"
        } else {
          s"Positional(${cf.cpLoss}cp)"
        }
