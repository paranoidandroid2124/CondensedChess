  ...
  def analyze(fen: String, board: Board, color: Color, mainLine: VariationLine, threatLine: Option[VariationLine], explicitPlanId: Option[String] = None): List[PreventedPlan] = {
    val threatPlans = threatLine.flatMap { threat =>
       val scoreDiff = PerspectiveMath.improvementForMover(
        ...
        if (scoreDiff > 50) {
         val finalPlanId = explicitPlanId.getOrElse("forcing_threat")
         ...
         Some(PreventedPlan(
           ...
           preventedThreatType = inferThreatType(Some(finalPlanId)),
           ...
           sourceScope = lila.chessjudgment.model.FactScope.ThreatLine,
...
class ActivityAnalyzerImpl extends ActivityAnalyzer {
  ...
  private def checkBadBishop(board: Board, piece: chess.Piece, square: chess.Square): Boolean = {
    piece.role == Bishop &&
      TacticalRelationEvidence.isConstrainedBadBishop(board, piece.color, square)
  }
}
...
class StructureAnalyzerImpl extends StructureAnalyzer {
  ...
  def analyze(board: Board): List[WeakComplex] = {
    val structures = for {
      ...
      kind <- List(WeakComplexKind.Holes, WeakComplexKind.DoubledPawns, WeakComplexKind.BackwardPawn, WeakComplexKind.HangingPawns)
    } yield (color, kind)

    structures.flatMap { case (color, kind) =>
      val squares = kind match {
        case WeakComplexKind.Holes => detectHoles(board, color)
        case WeakComplexKind.DoubledPawns => detectDoubled(board, color)
        case WeakComplexKind.BackwardPawn => detectBackward(board, color)
        case WeakComplexKind.HangingPawns => detectHanging(board, color)
      }
      if (squares.nonEmpty) Some(WeakComplex(color, squares, false, kind)) else None
    }
  }
  ...
  def detectPositionalFeatures(board: Board, color: Color): List[PositionalTag] = {
    ...
    ourBishops.squares.foreach { bSq =>
      ...
      // Require central pawn commitment before tagging.
...
class PracticalityScorerImpl extends PracticalityScorer {
  def score(
    ...
    val myWeaknessPenalty = myWeaknesses.map { w =>
       w.kind match {
         case WeakComplexKind.Holes => -20.0
         case WeakComplexKind.DoubledPawns => -15.0
         case WeakComplexKind.BackwardPawn => -25.0
         case WeakComplexKind.HangingPawns => -10.0
       }
    }.sum
    ...
    val biasFactors = List(
      BiasFactor("mobility", mobilityScore),
      BiasFactor("forgiveness", forgivenessPenalty),
      BiasFactor("structure", badBishopPenalty + myWeaknessPenalty)
