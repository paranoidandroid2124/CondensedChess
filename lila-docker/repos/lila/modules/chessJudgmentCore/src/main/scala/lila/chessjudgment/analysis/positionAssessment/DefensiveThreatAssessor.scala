 ...
 */
object DefensiveThreatAssessor:
  ...
  private def firstLegalStep(
    fen: String,
    pv: PvLine
  ): Option[TacticalRelationEvidence.BoundedReplayStep] =
  ...
  private def adjustThresholds(positionAssessment: PositionAssessment, multiPv: List[PvLine]): ThresholdConfig =
