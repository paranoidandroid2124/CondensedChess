...
import lila.chessjudgment.model.{ FingerprintFormat, PlanMatch, PlanSequenceSummary, TransitionType }
import lila.chessjudgment.model.strategic.{ PlanAbortReason, PlanContinuity, PlanLifecyclePhase }
import _root_.chess.Color

/**
 * Tracks strategic continuity per color for the live move judgment path.
 */
case class PlanStateTracker(
  ...
  private def continuityKey(c: PlanContinuity): String =
    c.planId.map(_.toLowerCase).getOrElse("")
          ...
          seq.transitionType match
            case TransitionType.ForcedPivot if isPrimary =>
              c.copy(
                ...
                abortedReason = Some(PlanAbortReason.ForcedPivot)
              )
...
object PlanStateTracker:
  ...
  private[plan] val defaultHistory: Map[Color, ColorPlanState] =
