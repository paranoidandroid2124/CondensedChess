...
import lila.chessjudgment.analysis.positionAssessment.{ PawnPlayAnalysis, PositionAssessment, ThreatAnalysis, TensionPolicy }
import lila.chessjudgment.analysis.PlanTaxonomy.{ PlanKind, PlanTheme }
...
case class IntegratedContext(
    evalCp: Int,
    positionAssessment: Option[PositionAssessment] = None,
  ...
  def phase: String = phaseEnum match
    case lila.chessjudgment.analysis.positionAssessment.GamePhaseType.Opening    => "opening"
    case lila.chessjudgment.analysis.positionAssessment.GamePhaseType.Middlegame => "middlegame"
    case lila.chessjudgment.analysis.positionAssessment.GamePhaseType.Endgame    => "endgame"
  def phaseEnum: lila.chessjudgment.analysis.positionAssessment.GamePhaseType =
    positionAssessment.map(_.gamePhase.phaseType).getOrElse(lila.chessjudgment.analysis.positionAssessment.GamePhaseType.Middlegame)
...
object PlanMatcher:
  ...
  def matchPlans(motifs: List[Motif], ctx: IntegratedContext, side: Color): PlanScoringResult =
    ...
    val openingRaw =
      Option.when(ctx.phaseEnum == lila.chessjudgment.analysis.positionAssessment.GamePhaseType.Opening)(
  ...
  def applyCompatWithEvents(
    ...
    if ctx.phaseEnum == lila.chessjudgment.analysis.positionAssessment.GamePhaseType.Opening then
  ...
  private def structureMatches(ctx: IntegratedContext, id: StructureId): Boolean =
