      ...
      oppKing.foreach { ok =>
        ...
        if (isDirect || isDistant || isDiagonal) {
          facts += Fact.Opposition(
            ...
            isDirect = isDirect, 
            oppositionType =
              if (isDirect) EndgameOppositionType.Direct
              else if (isDistant) EndgameOppositionType.Distant
              else EndgameOppositionType.Diagonal,
            scope = scope
          )
        }
      }
      ...
      (myKing, oppKing) match {
        case (Some(mk), Some(ok)) =>
          ...
          facts += Fact.Opposition(
            ...
            distance = Math.max(fileDiff, rankDiff),
            isDirect = isDirect,
            oppositionType = endgame.oppositionType,
            scope = scope
          )
        case _ => ()
      }
