...
private[commentary] enum InvestmentTruthPhase:
  case FirstInvestmentCommitment
  case CompensationMaintenance
  case ConversionFollowthrough

private[commentary] enum TruthOwnershipRole:
...
private[commentary] enum TruthVisibilityRole:
  case PrimaryVisible
  case SupportingVisible
  case Hidden

private[commentary] enum TruthSurfaceMode:
...
private[commentary] enum TruthExemplarRole:
  case VerifiedExemplar
  case ProvisionalExemplar
  case NonExemplar

private[commentary] enum CommitmentEvidenceProvenance:
  case CurrentMaterial
  case CurrentSemantic
  case AfterSemantic
  case LegacyShell

private[commentary] enum FailureInterpretationMode:
  case TacticalRefutation
...
private[commentary] enum MoveQualityVerdict:
  case Best
  case Acceptable
  case Inaccuracy
  case Mistake
...
private[commentary] final case class MoveQualityFact(
...
private[commentary] final case class BenchmarkFact(
...
private[commentary] final case class TacticalFact(
...
private[commentary] final case class MaterialEconomicsFact(
...
private[commentary] final case class FreshCommitmentEvidence(
...
private[commentary] final case class PunishConversionFact(
...
private[commentary] final case class FailureInterpretationFact(
...
private[commentary] final case class DifficultyNoveltyFact(
...
private[commentary] final case class MoveTruthFrame(
