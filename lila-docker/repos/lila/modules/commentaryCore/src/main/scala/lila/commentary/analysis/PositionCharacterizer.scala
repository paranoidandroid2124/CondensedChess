...
object PositionCharacterizer:
  ...
  def characterize(
    pos: Position,
    features: List[lila.commentary.model.strategic.PositionalTag],
    evalCp: Option[Int],
    material: String
  ): PositionNature =
  ...
  private def calculateKingSafetyPenalty(features: List[lila.commentary.model.strategic.PositionalTag]): Double =
    // Boost tension if King is exposed or under attack
    features.find {
      case lila.commentary.model.strategic.PositionalTag.MateNet(_) => true
      case lila.commentary.model.strategic.PositionalTag.KingStuckCenter(_) => true
      case lila.commentary.model.strategic.PositionalTag.WeakBackRank(_) => true
      case _ => false 
    } match
      case Some(_) => 0.3
      case None => 0.0
