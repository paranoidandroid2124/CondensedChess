...
private[commentary] object GameChronicleCompressionPolicy:
  ...
  private[commentary] final case class ChronicleRenderSurface(
      primary: QuestionPlan,
      secondary: Option[QuestionPlan],
  ...
  private[commentary] final case class ChronicleQuietSupportTrace(
      applied: Boolean,
      rejectReasons: List[String],
      composerTrace: QuietStrategicSupportComposer.QuietStrategicSupportTrace
  )

  private[commentary] final case class ChronicleRenderArtifact(
      narrative: String,
  ...
  private[commentary] def renderWithTrace(
      ctx: NarrativeContext,
      parts: CommentaryEngine.HybridNarrativeParts,
      strategyPack: Option[StrategyPack] = None,
      truthContract: Option[DecisiveTruthContract] = None
  ...
  private[commentary] def renderPlanSurface(
    ...
    sanitizePrimaryClaim(surface.primary, ctx).flatMap { claim =>
      if supportedLocalSurfaceOnly(surface.primary) then
        Some(
          ChronicleRenderArtifact(
            narrative = claim,
            quietSupportTrace =
              ChronicleQuietSupportTrace(
                applied = false,
                rejectReasons = List("supported_local_claim_only"),
                composerTrace = surface.quietSupportTrace
              )
          )
        )
      else {
  ...
  private def isLowValueSentence(text: String): Boolean =
    ...
    LiveNarrativeCompressionCore.isLowValueNarrativeSentence(text) ||
      ...
      low.startsWith("further probe work still targets") ||
      ...
      low.contains("probe work still targets") ||
