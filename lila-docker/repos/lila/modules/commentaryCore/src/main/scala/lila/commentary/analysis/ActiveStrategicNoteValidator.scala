...
private[commentary] object ActiveStrategicNoteValidator:
  ...
  def validate(
    ...
    val surfaceValidation = PlayerProseBoundary.validate(candidateText)
    ...
    val compensationContractExpected = ActiveCompensationPolicy.noteExpected(surface)
    ...
    val plannerWhyNowGrounded =
      resolvedPlannerPrimaryKind.contains(lila.commentary.model.authoring.AuthorQuestionKind.WhyNow) &&
        (
    ...
    val plannerSupportSatisfied =
      plannerMinimumContract.exists { contract =>
        contract.supportTexts.exists(phraseCovered(normalizedText, _)) ||
          (
            contract.questionKind == lila.commentary.model.authoring.AuthorQuestionKind.WhyNow &&
              plannerWhyNowGrounded
          ) ||
          (
    ...
    val baseHardReasons =
      List(
        ...
        Option.when(trimmed.nonEmpty && PlayerProseBoundary.looksJsonWrapper(trimmed))("json_wrapper_unparsed"),
        Option.when(trimmed.nonEmpty && PlayerProseBoundary.looksTruncated(trimmed))("truncated_output"),
