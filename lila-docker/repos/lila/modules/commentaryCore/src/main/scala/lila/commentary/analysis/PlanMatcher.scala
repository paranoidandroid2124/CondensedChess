...
case class IntegratedContext(
  ...
  def phase: String = phaseEnum match
    case lila.commentary.analysis.L3.GamePhaseType.Opening    => "opening"
    case lila.commentary.analysis.L3.GamePhaseType.Middlegame => "middlegame"
    case lila.commentary.analysis.L3.GamePhaseType.Endgame    => "endgame"
  def phaseEnum: lila.commentary.analysis.L3.GamePhaseType =
    classification.map(_.gamePhase.phaseType).getOrElse(lila.commentary.analysis.L3.GamePhaseType.Middlegame)
...
object PlanMatcher:
  ...
  def matchPlans(motifs: List[Motif], ctx: IntegratedContext, side: Color): PlanScoringResult =
    ...
    val openingRaw =
      Option
        .when(ctx.phaseEnum == lila.commentary.analysis.L3.GamePhaseType.Opening)(
  ...
  def applyCompatWithEvents(
    ...
    if ctx.phaseEnum == lila.commentary.analysis.L3.GamePhaseType.Opening then
