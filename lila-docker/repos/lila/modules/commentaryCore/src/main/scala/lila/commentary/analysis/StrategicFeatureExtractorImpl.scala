...
import lila.commentary.model.{ Motif, ExtendedAnalysisData, FactScope, PositionNature }
import lila.commentary.model.strategic.{ VariationLine, VariationTag, PreventedPlan, PositionalTag, EndgameFeature, EndgamePatternState }
import lila.commentary.analysis.strategic._
...
class StrategicFeatureExtractorImpl(
  ...
  def extract(
      ...
      case _ =>
        ...
        if (detectedThreats.nonEmpty) {
          ...
          val threatLine = VariationLine(
            ...
            mate = None,
            tags = Nil,
            parsedMoves = lila.commentary.analysis.MoveAnalyzer.parsePv(fen, detectedThreats.flatMap(_.move).take(3))
          )
          val causalThreatOpt = lila.commentary.analysis.ThreatExtractor.extractThreatConcept(fen, color, threatLine)
    ...
    val counterfactual = playedMove.flatMap { move =>
      ...
      selectedUserLine.flatMap { ul =>
        if (bestVar.moves.headOption != Some(move)) {
          ...
          val cpLoss = lila.commentary.analysis.PerspectiveMath.cpLossForMover(
            color,
            normalizedBestScore,
            normalizedUserScore
          )
          
          Some(lila.commentary.analysis.CounterfactualAnalyzer.createMatchNormalized(
            fen = fen,
            userMove = move,
    ...
    val analyzedVars = lila.commentary.analysis.MoveAnalyzer.analyzeVariations(fen, sortedVarsParsed, threatLineRaw)
    ...
    val candidates = sortedVarsParsed.take(3).map { candidateVar =>
      ...
      val candMotifs = lila.commentary.analysis.MoveAnalyzer.tokenizePv(fen, candidateVar.moves)
      ...
      lila.commentary.model.strategic.AnalyzedCandidate(
        move = moveStr,
        score = candScore,
        motifs = candMotifs,
  ...
  private def deriveConceptSummary(
    ...
    nature.natureType match {
      case lila.commentary.model.NatureType.Static => concepts += "Positional battle"
      case lila.commentary.model.NatureType.Dynamic => concepts += "Dynamic play"
      case lila.commentary.model.NatureType.Chaos => concepts += "Tactical complexity"
      case lila.commentary.model.NatureType.Transition => concepts += "Transitional phase"
    }
  ...
  private def deriveDualIntent(
    ...
    lila.commentary.model.strategic.MoveIntent(immediate, downstream)
  }
