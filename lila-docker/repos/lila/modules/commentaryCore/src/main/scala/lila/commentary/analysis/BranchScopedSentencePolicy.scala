...
import lila.commentary.model.authoring.{ NarrativeOutline, OutlineBeat }
...
object BranchScopedSentencePolicy:
  ...
  def hasUsableCitation(beat: OutlineBeat): Boolean =
    LineScopedCitation.hasInlineCitation(beat.text) ||
      (beat.kind == lila.commentary.model.authoring.OutlineBeatKind.Evidence && LineScopedCitation.hasConcreteSanLine(beat.text))
