...
import lila.commentary.model.{ Motif, PlanMatch, PositionNature }
import lila.commentary.model.authoring.PlanHypothesis
import lila.commentary.model.strategic._
import lila.commentary.model.structure.{ PlanAlignment, StructureProfile }

// The SSOT (Single Source of Truth) for AI Prompt Generation
case class ExtendedAnalysisData(
  ...
  def toContext: lila.commentary.analysis.IntegratedContext = {
    integratedContext.getOrElse {
      lila.commentary.analysis.IntegratedContext(
        evalCp = evalCp,
