...
private[commentary] object UserFacingSignalSanitizer:
  ...
  private val clubPlayerRegexRewrites: List[(Regex, String)] = List(
    ...
    """(?i)\bdelayed recovery only works if\b""".r -> "waiting before trying to recover the material makes the most sense while",
    """(?i)\bdelayed recovery\b""".r -> "waiting before trying to recover the material",
