...
object MoveAnalyzer:
  ...
  private val logger = lila.log("commentary.moveAnalyzer")
  ...
  private def dualRunParityEnabled: Boolean =
    boolSetting(
      "commentary.moveAnalyzer.dualRunParity",
      "AI_MOVE_ANALYZER_DUAL_RUN_PARITY",
      default = true
    )

  private def parityLogEvery: Int =
    intSetting("commentary.moveAnalyzer.parityLogEvery", "AI_MOVE_ANALYZER_PARITY_LOG_EVERY", default = 200, min = 20, max = 5000)
   ...
   */
  def parsePv(initialFen: String, pv: List[String]): List[lila.commentary.model.strategic.PvMove] =
    Fen.read(chess.variant.Standard, Fen.Full(initialFen)).map { initialPos =>
      val (_, pvMoves) = pv.foldLeft((initialPos, List.empty[lila.commentary.model.strategic.PvMove])) {
        case ((pos, acc), uciStr) =>
          ...
          uciMoveOpt.flatMap(uciMove => pos.move(uciMove).toOption) match
            case Some(mv) =>
              ...
              val pvMove = lila.commentary.model.strategic.PvMove(
                uci = uciStr,
                san = san,
                from = mv.orig.key,
                to = mv.dest.key,
