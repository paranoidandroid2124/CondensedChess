...
import lila.commentary.{ DirectionalTargetReadiness, StrategyDirectionalTarget, StrategyPack, StrategyPieceMoveRef, StrategyPieceRoute, StrategySidePlan }
import lila.commentary.model.{ ExtendedAnalysisData, NarrativeContext }
...
object StrategyPackBuilder:
  ...
  def promptHints(pack: StrategyPack): List[String] =
    ...
    val routeHints = pack.pieceRoutes.take(2).map { r =>
      val routeLead =
        if r.surfaceMode == lila.commentary.RouteSurfaceMode.Exact then r.route.mkString("-")
