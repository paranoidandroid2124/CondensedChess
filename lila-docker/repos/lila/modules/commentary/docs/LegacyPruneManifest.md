...
PawnAdvance-0 authority lives in `StoryInteractionLaw.md`. It opens only narrow `Scene.PawnAdvance` over `PawnAdvanceProof` for bounded progress by an already-passed pawn; legacy broad PawnTactic, `Tactic.PawnPush`, promotion threat, unstoppable pawn, winning endgame, conversion, pawn race, strategy, production API, public route, and public/user-facing LLM paths do not return.
PawnStop-0 authority lives in `StoryInteractionLaw.md`. It opens only narrow `Scene.PawnStop` over `PawnStopProof` for bounded immediate next-square stop of an already-passed pawn; legacy broad pawn defense, promotion stop, permanent stop, tablebase draw, best defense, only move, endgame hold, conversion stopped, pawn race, king route, opposition, broad pawn strategy, production API, public route, and public/user-facing LLM paths do not return.
...
PromotionThreat-0 authority lives in `StoryInteractionLaw.md`. It opens only narrow `Scene.PromotionThreat` over `PromotionThreatProof` for a legal pawn move creating a legal next-move promotion route on the exact after-board; legacy actual Promotion Story, unstoppable pawn, cannot-be-stopped, winning endgame, conversion, tablebase, pawn race, best move, only move, forced win, no-counterplay, production API, public route, and public/user-facing LLM paths do not return.
Promotion-1 authority lives in `StoryInteractionLaw.md`. It opens only diagnostic `PromotionProof` for a legal non-capturing pawn promotion event on the exact board; legacy promotion material result, capture promotion, conversion, winning endgame, decisive advantage, best move, tablebase, production API, public route, and public/user-facing LLM paths do not return.
Promotion-2 authority lives in `StoryInteractionLaw.md`. It opens only the named `ScenePromotion` writer for proof-backed non-capturing promotion identity; legacy promotion-threat-by-promotion, Scene.Material-by-promotion, winning, conversion, tablebase, renderer, LLM, production API, public route, and public/user-facing LLM paths do not return.
Promotion-3 authority lives in `StoryInteractionLaw.md`. It opens only the negative corpus for actual non-capturing promotion proof or silence; legacy near-promotion-as-promotion, capture-promotion-as-opened-scope, material-result-as-promotion, winning, conversion, tablebase, renderer, LLM, production API, public route, and public/user-facing LLM paths do not return.
Promotion-4 authority lives in `StoryInteractionLaw.md`. It reuses only the existing `EngineCheck` sidecar for same-board `ScenePromotion` rows; legacy engine-created-promotion, engine-says promotion, eval-number wording, tablebase result, best move, only move, winning endgame, forced win, production API, public route, and public/user-facing LLM paths do not return.
Promotion-5 authority lives in `StoryInteractionLaw.md`. It opens only StoryTable collision checks for actual non-capturing Promotion rows; legacy promotion-steals-material, promotion-steals-threat, legacy threat-steals-actual-promotion, material-result-as-promotion, capture-result-as-promotion, renderer, LLM, production API, public route, and public/user-facing LLM paths do not return.
Promotion-6 authority lives in `StoryInteractionLaw.md`. It opens only the internal `ExplanationPlan` handoff for selected uncapped Lead actual Promotion rows with `promotes_pawn`; legacy promotion-explanation-as-win, conversion, decisive, best move, only move, forced win, tablebase win, unstoppable pawn, material gain, renderer, LLM, production API, public route, and public/user-facing LLM paths do not return.
Promotion-7 authority lives in `StoryInteractionLaw.md`. It opens only the deterministic renderer template `{route} promotes the pawn.` for selected uncapped Lead actual Promotion ExplanationPlans; legacy promotion-renderer-as-win, winning endgame, decisive, conversion, best move, only move, forced win, tablebase win, material gain, unstoppable pawn, LLM, production API, public route, and public/user-facing LLM paths do not return.
Promotion-8 authority lives in `StoryInteractionLaw.md`. It opens only the existing 8B LLM smoke boundary for selected actual Promotion rendered text contract fields; legacy promotion-llm-as-win, raw Story, raw PromotionProof, BoardFacts, EngineCheck, raw PV, proofFailures, new move, new line, winning, conversion, tablebase, material gain, production API, public route, and public/user-facing LLM paths do not return.
Promotion Closeout authority lives in `StoryInteractionLaw.md`. It closes Promotion as one legal non-capturing pawn promotion event with one proof home, one Story label, and one speech key; legacy promotion-closeout-as-win, capture promotion, PromotionThreat theft, PawnAdvance/PawnStop/PawnBreak theft, Material/Defense/Hanging/Line ownership theft, winning, conversion, decisive, tablebase, material gain, production API, public route, and public/user-facing LLM paths do not return.
PIH-0 authority lives in `StoryInteractionLaw.md`. It opens no new pawn meaning; it hardens existing `Scene.PawnAdvance` and `Scene.PawnStop` interactions so legacy promotion, conversion, broad pawn tactic, defense-stealing, tactic-stealing, production API, public route, and public/user-facing LLM paths do not return.
PIH-1 authority lives in `StoryInteractionLaw.md`. It opens only the Pawn Interaction Hardening fixture map over already-open rows and keeps legacy PromotionThreat, Promotion, PawnBreak, tablebase, endgame-result, production API, public route, and public/user-facing LLM paths closed.
PIH-2 authority lives in `StoryInteractionLaw.md`. It opens only StoryTable role stability over already-open pawn and collision rows, while legacy duplicate selector, promotion, conversion, defense-stealing, production API, public route, and public/user-facing LLM paths remain closed.
PIH-3 authority lives in `StoryInteractionLaw.md`. It opens only meaning ownership checks over already-open pawn, material, defense, and line/defender rows, while legacy promotion, conversion, endgame, defense-stealing, pawn-progress stealing, production API, public route, and public/user-facing LLM paths remain closed.
PIH-4 authority lives in `StoryInteractionLaw.md`. It opens only EngineCheck interaction checks over already-open PawnAdvance and PawnStop rows, while legacy engine-owned claims, raw PV, eval-number public values, best-move, only-move, tablebase-like, production API, public route, and public/user-facing LLM paths remain closed.
PIH-5 authority lives in `StoryInteractionLaw.md`. It opens only the close pawn false-positive negative corpus, while legacy promotion threat, promotion, long-term blockade, tablebase, opposition, pawn race, route-mismatch repair, stale/wrong-board engine evidence, production API, public route, and public/user-facing LLM paths remain closed.
PIH-6 authority lives in `StoryInteractionLaw.md`. It opens only downstream boundary smoke over already-open PawnAdvance and PawnStop rows, while legacy downstream public text for non-Lead, capped, refuted, promotion, unstoppable, winning, conversion, draw/hold, tablebase, best/only, forced, pressure, initiative, production API, public route, and public/user-facing LLM paths remains closed.
PIH-7 authority lives in `StoryInteractionLaw.md`. It opens only diagnostics boundary hardening over already-open PawnAdvance and PawnStop rows, while legacy public diagnostics, raw proof text, EngineCheck text, StoryTable debug text, helper-derived runtime authority, production API, public route, and public/user-facing LLM paths remain closed.
PIH Closeout authority lives in `StoryInteractionLaw.md`. It opens no new pawn meaning, Story family, proof home, Story writer, claim key, renderer wording, LLM behavior, production API, public route, or public/user-facing LLM paths, while legacy promotion, pawn-break, tablebase, pawn-race, king-route, opposition, mixed proof authority, sibling claim-home invasion, and helper-authority paths remain closed.
