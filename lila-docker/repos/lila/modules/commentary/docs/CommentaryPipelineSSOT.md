...
- Canonical audit for commentary-analysis in `modules/commentary`
- Canonical audit for backend prompt/polish behavior that affects commentary
- Canonical audit for `app/controllers/CommentaryController.scala` and
...
17. Chronicle conclusion quality is carried by a deterministic game-level
    binder inside `CommentaryEngine`, not by an AI-only summary layer.
...
- opening / middlegame / endgame transitions are narrated from the shared
  strategic state, not from an AI-only interpretation layer
...
5. `MoveReviewCompressionPolicy` is planner-first:
   - `primary` owns `claim`
...
6. if no certified planner slot survives, `MoveReviewExplanationBuilder` may
   ...
   - opening name resolution is FEN/EPD-first at the existing
     `OpeningReference` boundary: `OpeningNameLookup` normalizes piece
     placement, side-to-move, castling, and legal en-passant into an EPD key,
     excluding halfmove/fullmove counters, then supplies canonical static
     `eco/name` labels while `OpeningExplorerClient` keeps explorer statistics
     (`totalGames`, `topMoves`, `sampleGames`) as separate enrichment
   - `OpeningExplorerClient.fetchMasters(fen)` and full-game
     `openingRefsByFen` both continue to return/consume `OpeningReference`;
     static canonical labels may survive explorer miss/failure, but they do not
     create a parallel opening carrier or sequence-string authority path
   - current-move legality and post-move FEN are verified by legal replay; the
     MoveReview lane no longer owns a separate board-primitive extractor
    - opening prose is gated through the single
      `NarrativeContext.openingGoalEvaluation` carrier, computed once by
      `NarrativeContextBuilder` after legal current-move replay from the
      current FEN; `OpeningReference` / `OpeningEvent` metadata may name the
      opening, but the opening label alone is never enough to admit
      instructional prose
   - tactical and endgame admission consumes the canonical `Fact` / `Motif`
   ...
   - the prose may use that interpretation to explain current-move purpose,
    ...
    - `MoveReviewPolishSlots.sourceKind` records the selected visible lane; the
      structured `moveReviewExplanation` is carried on those same slots only
      when `sourceKind=basic_move_explanation`, so the API does not recompute
      the explanation and a planner-owned surface does not carry a separate
...
- Bookmaker / Chronicle may omit weak strategic packaging entirely:
  ...
  - packet admission is now witness-backed rather than family-label-only:
    `PlayerFacingClaimProof` requires a concrete backend-only owner
...
1. `CommentaryApi` selects the active-note subset from Chronicle moments.
   - the same Chronicle build also keeps a backend-only per-ply
     `DecisiveTruthContract` sidecar from
     `CommentaryEngine.generateGameArcDiagnostic`; `CommentaryApi.attachActiveStrategicNotes`
...
- `sourceMode = rule / ai_polished / omitted` stays externally stable.
- `ai_polished` means wording polish over a deterministic draft, not fresh
...
- active/support is no longer a state-summary coaching card:
  ...
  - active-note visible surface is stricter than active-note selection:
    ...
    - `CommentaryApi` may not auto-attach dossier / refs from raw carriers; the note
...
- `modules/commentary/src/main/CommentaryApi.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/ActiveBranchDossierBuilder.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/ActiveStrategicCoachingBriefBuilder.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/ActiveStrategicNoteValidator.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/StrategicSentenceRenderer.scala`
- `modules/commentaryAi/src/main/scala/lila/commentary/ActiveStrategicPrompt.scala`
...
- outline authoring is also question-first:
  ...
  - `QuestionPlannerInputsBuilder` is the shared owner-path bundle for
    `NarrativeOutlineBuilder`, `MoveReviewCompressionPolicy`, and
...
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/AuthorQuestionGenerator.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/AuthorEvidenceBuilder.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/QuestionFirstCommentaryPlanner.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/EvidencePlanner.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/ProbeDetector.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/NarrativeOutlineBuilder.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/NarrativeOutlineValidator.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/NarrativeContextBuilder.scala`
...
- backend-only helper `RestrictedDefenseConversionProof` gates only
...
- backend-only helper `CounterplayRestraintProof` gates only
...
- backend-only helper `TwoAxisBindProof` gates only the first B3 slice:
  `RestrictionProphylaxis` strategic experiments with
...
- backend-only helper `LocalFileEntryProof` gates only the first
  B4b slice:
  `RestrictionProphylaxis` strategic experiments with
  `BreakPrevention` / `KeySquareDenial` when current `PreventedPlan` evidence
...
- exact-board status:
  `LocalFileEntryProofBoundaryTest` now includes the
...
- scope:
  backend-only `RouteNetworkBindProof` now hardens and narrows the
...
- runtime path:
  ...
  `LocalFileEntryProof` and before `HeavyPieceLocalBindValidation`,
...
- scope:
  the same `RouteNetworkBindProof` lane is still the only
...
- boundary:
  ...
  in `CommentaryProgramMap.md` / `CommentaryTrustBoundary.md`, not here
- hard-fail reasons:
...
- `AI_PROVIDER` default remains `openai`
- `AI_PROVIDER_ACTIVE_NOTE` default remains `gemini`
...
- real-PGN signoff runner in `modules/commentaryTools/src/test/scala/lila/commentary/tools/realpgn/RealPgnNarrativeEvalRunner.scala`
- signoff report merge in `modules/commentaryTools/src/test/scala/lila/commentary/tools/realpgn/RealPgnNarrativeEvalReportMerge.scala`
...
- live owners:
  ...
  - `RestrictedDefenseConversionProof`
...
Current maintained markdown contracts in `modules/commentary/docs`:
...
- `CommentaryTrustBoundary.md`
  - trust-risk inventory, source-backed authority examples, and fail-closed
    boundary rationale
...
- `modules/commentary/src/main/CommentaryApi.scala`
- `modules/commentaryAi/src/main/scala/lila/commentary/ActiveStrategicPrompt.scala`
- `modules/commentaryAi/src/main/scala/lila/commentary/GeminiClient.scala`
- `modules/commentaryAi/src/main/scala/lila/commentary/OpenAiClient.scala`
- `modules/commentaryAi/src/main/scala/lila/commentary/PolishPrompt.scala`
- `modules/commentaryAi/src/main/scala/lila/commentary/StrategicPuzzlePrompt.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/ActiveBranchDossierBuilder.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/ActiveStrategicCoachingBriefBuilder.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/ActiveStrategicNoteValidator.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/BookStyleRenderer.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/MoveReviewCompressionPolicy.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/MoveReviewPolishSlots.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/MoveReviewStrategicLedgerBuilder.scala`
- `modules/commentaryCore/src/main/scala/lila/commentary/analysis/CommentaryEngine.scala`
