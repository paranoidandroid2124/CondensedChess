...
This document is the canonical trust-boundary map for the Chesstory
...
This document does not replace the runtime audit or truth boundary.
...
- `CommentaryTruthBoundary.md`
  - canonical release/signoff truth boundary
...
- `PlanKind`: the chess plan being reviewed.
- `proofSource`: the proof lane that produced a packet.
- `proofFamily`: the proof contract name, checked by `ProofContractRules`.
- `PlannerOwnerKind`: the planner question-owner lane.
- `reviewGroup`: a source-review intake bucket.
- `AdmissionUnit`: one repeatable review unit over a `PlanKind`,
  `proofSource`, and `proofFamily`.

Blockers for a wrong proof contract use `proof:*_contract_mismatch`. Witness
or extraction failures may still use `owner:*` when the planner/source row has
no exact witness, but they do not rename a layer to `family`.
...
- Current mid-layer boundary note:
...
- Packet pilot status on 2026-04-03:
  ...
  (`slows X before it gets started`). When both a named resource and raw
  break-square witness exist, the named resource owns this family surface;
  generic prophylaxis praise, whole-position no-counterplay, move-order
  fragility, heavy-piece relabel, and support-only reinflation remain
  fail-closed. Active and whole-game replay stay closed for this family too.
  ...
  `proofFamily=target_focused_coordination`,
  `scope=PositionLocal`,
  ...
  `trade_key_defender` remains blocked on exact owner-path proof:
  the exact `d4e6|f7e6` control can now carry private post-trade owner seed,
  continuation, and structure-transition witness, but trust boundary still
...
updated when later trust-boundary work changes the audited behavior.

### CTH-A — Trust Boundary
...
Status:
core complete on the current trust-boundary tranche; now maintenance-only for
...
- the current strategic-discovery evidence carrier is
  `lila.commentary.model.NarrativeContext`
  with:
...
`RestrictedDefenseConversionProof.scala`. It records:
...
field, or surface phrase class was added. The work stays in test/tooling and reuses the
...
- contract/builder alignment:
  `RestrictedDefenseConversionProofBoundaryTest` checks that the
...
- existing trust harnesses:
  `RestrictedDefenseConversionProofTest`,
  `RestrictedDefenseConversionProofBoundaryTest`,
...
containment. No public payload/schema field or new surface phrase class was added.
...
| `bindArchetype` | stable role label such as `break_plus_entry`, `break_plus_network`, `entry_plus_mobility`, `file_plus_king_route` | `PlanTaxonomy` subplans, `PlanEvidenceEvaluator` ontology family, `ThemePlanProbePurpose` |
...
planner/replay containment. No public payload/schema field or new surface phrase class
...
- implementation site:
  `TwoAxisBindProof.scala` evaluated from
...
- positive owner cap:
  ...
  on its own lane, not as a new B3 planner owner kind
...
- targeted A/CQF seam review found no new loophole across fallback rewrite,
  truth-boundary re-entry, whole-game support reuse, or cross-surface divergence;
...
- the first B4 runtime draft left a real reinflation seam:
  `LocalFileEntryProof.certifiedSurfacePair(ctx)` was reading raw
...
B4 fix-up boundary closed in this step:
...
- targeted compile: `commentary/compile`
- B4 broad validation:
  `LocalFileEntryProofBoundaryTest`
- regression packs:
  `TwoAxisBindProofBoundaryTest`,
  `CounterplayRestraintProofBoundaryTest`,
  `SurfaceReplayParityTest`,
...
- architectural boundary:
  ...
  owner lane, public payload/schema, or surface phrase class
...
- planner surface fix-up:
  ...
  `RouteNetworkBindProof.certifiedSurfaceNetwork` itself now fails
...
- boundary rule:
  no new runtime path, payload/schema, surface phrase class, replay lane, or owner lane
...
| `whole_game_wrapper_leak` | support-only route residue can seed wrap-up or decisive-wrapper rhetoric | whole-game / wrap-up are already closed to B6, truth boundary bans support-only decisive wrappers | keep wrapper reuse explicitly unsafe until a later review proves exact carried-chain consumption and no new decisive wrapper seed | reuse `surface_reinflation_case` at `2rq1rk1/pp3ppp/2n1pn2/3p4/3P4/P1P1PN2/1PQ2PPP/2R2RK1 w - - 0 24` across planner / replay / whole-game |
...
- no new ingress, planner lane, replay lane, planner owner kind, payload field, or
  surface phrase class was added in this campaign
- the session used the existing engine/replay helpers plus the new
  `NamedRouteSurfaceLedger.scala` test/tooling runner to refresh the
  current control, candidate pool, and blocker pack on the same branch
- the 2026-04-03 rerun wrote
  `modules/commentary/tmp/named-route-validation-matrix-20260403.tsv`
...
| `holdable_simplification` | simplification happens, but the defender still reaches a static shell with no real progress route conceded | B1 / B3 / B4 fortress-risk and static-hold heuristics | require `technicalGain` to show more than liquidation: one blocker removed and one bounded next task opened | transition near-miss exact FEN `8/5pk1/3b2p1/3P4/1P3P2/6P1/5BK1/8 w - - 0 45` from `TwoAxisBindProofBoundaryTest.scala` |
...
- it can stay on the existing architecture and planner `WhyThis` lane without
  opening a new public payload, planner owner kind, or replay surface
...
- a future B7 slice does not yet prove that a new owner lane is necessary, but
  it does require new private/backend-only proof on the existing
  architecture
...
- generic `PlannerOwnerKind.MoveDelta` is not a sufficient contract by itself; it
...
| --- | --- |
| exact branch exists, but no task-shift carrier/proof exists | `deferred` |
...
| any design pressure appears for new public schema, new owner lane, or replay-positive reuse | keep `B7` closed; no proof beyond private freeze |
...
- because `no_b7_positive_control` remains live, future B7 reopen now requires
  ...
  - implement private/backend-only proof only
...
No runtime slice, new payload field, new owner lane, or new surface phrase class is
...
- the current pack is large enough to hold the existing trust-boundary line
...
- fallback truth rewrite boundary
...
1. CTH-A is now core complete, but only as a trust-boundary tranche; it does
...
1. CTH-A core slices
   - complete:
     fallback truth rewrite boundary, Active contract alignment, authority
...
4. opening/endgame replay claim boundary as a fixed regression target
...
## Strategic Claim Authority Ladder

The current B/C trust-boundary experiment uses one internal authority tiers in
`ClaimAuthorityPolicy`:

- `CertifiedOwner`
  - exact owner path is still the strong surface.
  - current exact controls include B fixed-target / target-coordination
    position probes, B target-fixation move deltas, and C same-task
    favorable-simplification move deltas.
  - 2026-05-13 source-backed boundary also admits the Evans-Opsahl exact
    `carlsbad_fixed_target_probe` row when the certified non-tactical packet
    appears inside a forcing-defense scene; the narrow admission reason is
    `certified_position_probe_non_tactical_forcing_scene`.
  - 2026-05-13 source-backed boundary also admits the
    Boleslavsky-Nezhmetdinov exact `static_weakness_fixation` row through
    `proofSource=exact_target_fixation`, `scope=MoveLocal`, and
    `contract=subplan:static_weakness_fixation`. The released claim is limited
    to fixing d6 as the exact target on the proven branch.
- `SupportedLocal`
  - allowed only as weak, local wording when the packet is otherwise weak-main
...
- `TacticalVeto`
  - blunder, missed win, tactical refutation, tactical truth mode, or severe
    tactical counterfactual suppresses B/C strategic release.
  - ordinary tactical pressure alone is not enough for veto because it produced
    false positives on live quiet B/C controls.

The authority tiers now sits behind a global owner-proof catalog in
`ProofContractRules`. The catalog covers every current
`PlanTaxonomy.PlanTheme` and `PlanTaxonomy.PlanKind` with either a releasable
contract or an explicit `Deferred` / `BackendOnly` status. This is full-opening
preparation, not automatic full release: a family may produce player-facing
strategic prose only when the packet passes the contract, planner admission,
tactical-veto checks, and full surface validation.
...
`PlayerFacingClaimPacket` remains the canonical packet and now carries an
internal `proofTrace` with contract id/status, witness booleans,
...
`tmp/strategic_claim_authority_surface_ledger.tsv`. Filtered debug runs write `_subset`
artifacts so they cannot overwrite the canonical full matrix.

Current matrix scope:

- natural review rows from `TaskShiftProvingFixtures.reviewFixtures`
- curated B/C positive and blocker rows
- FEN-only screen rows from `StrategicIdeaFenFixtures` for additional B
  fixed-target, C simplification, and C attacker-trade candidates
- PV-backed priority rows copied from `PlanPriorityFenFixtureTest` for
  material-imbalance simplification, tactical-first Dragon, and compensation
  target-pressure boundaries
- source-backed exact rows copied from Stockfish intake for Evans-Opsahl 1950,
  Carlsen-Anand 2014 game 6, five natural IQP inducement rows
...
- controlled tactical-veto rows, plus tactical-first suppressed controls for
  rows that had no releasable B/C baseline

Current expanded matrix result on 2026-05-15:

- `CertifiedOwner=20`
  - examples:
    `B15A` / `B16B` current-position `carlsbad_fixed_target_probe`,
    `K09A` / `K09D` current-position
    `target_focused_coordination_probe`, `B21` / `B21A` target-fixation
    move deltas, `K09B` / `K09F` same-task simplification, the
    PV-backed `priority-MI1` simplification conversion candidate, and
    `source-salov-ljubojevic-1992-simplification-window`.
  - source-backed natural B examples:
    `source-evans-opsahl-1950` now materializes
    `proofSource=carlsbad_fixed_target_probe`, `scope=PositionLocal`, and
    planner-owned `WhatMattersHere` despite the non-tactical forcing-defense
    scene. `source-boleslavsky-nezhmetdinov-1950-static-weakness-fixation`
...
- `SupportedLocal=14`
  - examples:
    ...
    `source-pfleger-maalouf-1961-a6-a5-break-prevention`.
  - these rows must render only as `A local reading is that ...` and must not carry
...
- `Suppressed=42`
  - examples:
    `K03A`, `K08A`, `K08D`, `MI5`, `K09E`, `MI2`, `MI3`, all additional
    FEN-only screen rows, and most PV-backed priority rows including the
    tactical-first `priority-MR1` / `priority-MR2` / `priority-MR3` /
    `priority-TO1` controls. The tactical overlays on `priority-MR1`,
    `priority-MR2`, and `priority-TO1` are counted here too because their
    non-tactical baseline has no releasable B/C owner path; they remain
    tactical-first negative controls, not strict veto evidence.
    `break-prevention-missing-witness-control` and
    `break-prevention-rival-relabel-control` also stay suppressed.
    `prophylaxis-restraint-missing-witness-control` and
    `prophylaxis-restraint-rival-relabel-control` stay suppressed for the same
    exact-witness / relabel-boundary reason.
- `TacticalVeto=8`
  - examples:
    tactical overlays on `B15A`, `B16B`, `K09A`, `K09B`, `K09F`, and the
    softened `B15A` row, plus `break-prevention-tactical-veto` and
    `prophylaxis-restraint-tactical-veto`.
  - this label is now strict: it requires either an explicit
    `strategic_claim_tactical_veto` rejection or a non-tactical baseline that
    would have produced `CertifiedOwner` / `SupportedLocal`.
  - expected surface is omission or tactical-first factual fallback, with no
    B/C strategic leak.

The generated human review snapshot is
`tmp/strategic_claim_authority_surface_review.md`; the TSV matrix is
`tmp/strategic_claim_authority_surface_ledger.tsv`. The matrix TSV and review snapshot now
include `contractId`, `contractStatus`, and `contractFailures` so admitted rows
can be audited against the same backend owner-proof contract used by
runtime release.

The 2026-05-13 corpus pass also writes a source intake note at
`tmp/strategic_claim_source_witnesses.md`. Web/source-backed rows are
not authority until replayed into exact FEN / PV data and rerun through local
engine-backed validation. The exact replay path now exists in
`SourceWitnessCatalog` and `SourceReview`;
the runner writes
...
- `source-capablanca-golombek-1939-iqp-inducement` is a one-ply exact row:
  ply 45, FEN
  `r3r1k1/pp3pn1/2pq2pp/3p4/NP1P4/3QP2P/P4PP1/1RR3K1 w - - 0 23`,
  played `b4b5`, top PV prefix `b4b5 e8c8 b5c6 b7b6`,
  `source=iqp_inducement_probe`, `scope=MoveLocal`,
  `contract=subplan:iqp_inducement`, `release=SupportedLocal`.
- `source-botvinnik-vidmar-1936-iqp-multipv-screen` is also a one-ply exact
  row. The engine gate is
  `near_top_multipv_contains_played_top=b3a4_gap=6cp`, but exact IQP owner
  proof fails with `owner:iqp_not_induced`, so the verdict remains
  `reject_owner_missing`.
- `source-evans-opsahl-1950-iqp-inducement` is a one-ply exact row:
  ply 33, FEN
  `r3rnk1/1p3ppp/p1p5/3p2q1/PP1P2b1/2QBP3/3N1PPP/1R3RK1 w - - 3 17`,
  played `f1c1`, top PV prefix `f1c1 h7h5 b4b5 c6b5`,
  `source=iqp_inducement_probe`, `scope=MoveLocal`,
  `contract=subplan:iqp_inducement`, `release=SupportedLocal`.
- `source-alekhine-bogoljubow-1936-iqp-inducement` is a one-ply exact row:
  ply 20, FEN
  `rnb1k2r/pp3ppp/4p3/2pqP3/PbpPn3/2N2N2/1PQ1BPPP/R1B2RK1 b kq - 1 10`,
  played `e4c3`, top PV prefix `e4c3 b2c3 c5d4`,
  `source=iqp_inducement_probe`, `scope=MoveLocal`,
  `contract=subplan:iqp_inducement`, `release=SupportedLocal`.
- `source-najdorf-sergeant-1939-iqp-inducement` is a one-ply exact row:
  ply 23, FEN
  `r1b2rk1/pp2qppp/4p3/2nn4/3N4/2N1P3/PPQ2PPP/3RKB1R w K - 0 12`,
  played `c3d5`, top PV prefix `c3d5 e6d5`,
  `source=iqp_inducement_probe`, `scope=MoveLocal`,
  `contract=subplan:iqp_inducement`, `release=SupportedLocal`.
- `source-botvinnik-vidmar-1936-iqp-opening-inducement` is a one-ply exact
  row: ply 16, FEN
  `r1bq1rk1/pp1nbppp/4pn2/2pp2B1/2PP4/2NBPN2/PP3PPP/R2Q1RK1 b - - 1 8`,
  played `c5d4`, near-top source branch prefix
  `c5d4 e3d4 d5c4 d3c4`,
  `source=iqp_inducement_probe`, `scope=MoveLocal`,
  `contract=subplan:iqp_inducement`, `release=SupportedLocal`.
- `source-salov-ljubojevic-1992-simplification-window` is a one-ply exact
  row: ply 71, FEN
  `7k/p4qp1/8/1Q1pR3/3P1P2/2r3P1/7P/6K1 w - - 0 36`,
  played `b5d5`, top PV prefix `b5d5 f7d5 e5d5`,
  `source=simplification_window`, `scope=MoveLocal`,
  `contract=subplan:simplification_window`, `release=CertifiedOwner`.
- `source-boleslavsky-nezhmetdinov-1950-static-weakness-fixation` is a one-ply
  exact row: ply 19, FEN
  `rnbqr1k1/pp3pbp/3p1np1/2pP4/4P3/2N2N2/PP2BPPP/R1BQ1RK1 w - - 6 10`,
  played `f3d2`, top PV prefix
  `f3d2 b8a6 g1h1 a6c7 a2a4 b7b6 f2f3 d8e7 d2c4`,
  `source=exact_target_fixation`, `scope=MoveLocal`,
  `contract=subplan:static_weakness_fixation`, `release=CertifiedOwner`.
- `source-maderna-palermo-1955-static-weakness-fixation` is a one-ply exact
  row: ply 17, FEN
  `rnbq1rk1/pp3pbp/3p1np1/2pP4/4P3/2N2N2/PP2BPPP/R1BQK2R w KQ - 4 9`.
  The engine top line matches the source move, but no owner packet
  materializes; the row stays non-authority with
  `owner:root_vocabulary_or_extraction_gap`.
- `source-aronian-andreikin-2014-defender-trade` is a one-ply exact row:
  ply 33, FEN
...
- `source-karpov-unzicker-1974-break-prevention` reviews the 16.b4 to 24.Ba7
  ...
  `owner:break_prevention_capture_transform_recapture_unproven`, sixteen rows
  remain `owner:break_prevention_no_prevented_plan`, and four rows are also
  engine-source mismatches.
- `source-karpov-andersson-1975-hedgehog-break-screen` reviews the Hedgehog
  break screen around Black's `...b5` / `...d5` liberation attempts in
  Karpov-Andersson, Milan 1975. The fixed head row is rejected with
  `owner:break_prevention_no_prevented_plan`. Later window plies can
  materialize incidental `iqp_inducement_probe` `SupportedLocal` surfaces, but
  the admission-unit pass rejects them as
  `proof:break_prevention_contract_mismatch` because they do not align to
  `counterplay_axis_suppression` / `neutralize_key_break`.

The admitted fixed-intake source rows are exact-slice narrow: Evans-Opsahl
admits only at ply 25 for the source Carlsbad authority row, Carlsen-Anand game
6 admits only at ply 15, and Capablanca-Golombek admits only through the
separate ply-45 IQP row. Salov-Ljubojevic admits only through the fixed ply-71
Qxd5 simplification row. Boleslavsky-Nezhmetdinov admits only through the
...
incidental IQP materialization inside that source window is not allowed to count
as `break_prevention` admission. The current blocker split is sharper than the
older umbrella `owner:break_prevention_witness_missing`: fixed/source rows with
no current prevented-plan carrier report
`owner:break_prevention_no_prevented_plan`, route-denial rows with a surviving
same-destination capture route report concrete transform blockers such as
`owner:break_prevention_capture_transform_recapture_unproven`, and
contract-stealing rows report `proof:break_prevention_contract_mismatch`.
Other wider-window
positives remain review candidates
until selected as fixed intake rows and rerun through exact replay, engine
authority, matrix, and surface checks.

The same pass caught two surface issues before signoff:

- Chronicle fallback could append `Further probe work still targets ...` as
...
`planKindId` / `proofSource` / `proofFamily` unit: one controlled positive,
three negative controls, 2-5 natural source candidates, and 0-2 admitted
authority rows. The catalog writes
`tmp/strategic_admission_unit_catalog.tsv` and
`tmp/strategic_admission_unit_catalog.md`; it is a test/tooling artifact, not
runtime policy. The default queue is:

| plan kind | proof source | proof family | initial status |
| --- | --- | --- | --- |
| `break_prevention` | `counterplay_axis_suppression` | `neutralize_key_break` | `Releasable` as `SupportedLocal` proof target |
...
`neutralize_key_break` as proof family. The controlled positive releases only
as `SupportedLocal` with
the downgraded text
`A local reading is that this keeps ...c5 from coming right away.` The three
negative controls close as tactical veto, missing exact owner witness, and
rival/generic counterplay relabel. The initial Karpov-only natural source review covered
`source-karpov-unzicker-1974-break-prevention` and
`source-karpov-andersson-1975-hedgehog-break-screen`; that pass admitted
`0` natural `break_prevention` matrix rows. `BreakPreventionWitness` now
centralizes the live runtime witness extraction before owner-seed creation: the
candidate must be `FactScope.Now`, carry a normalized named break with local
counterplay loss (`counterplayScoreDrop > 0` or `mobilityDelta < 0`), align to
the break-prevention family, and retain branch / persistence / rival-release
diagnostics for exact proof. No source id is used as runtime policy, no broad
release follows, and `QuestionFirstCommentaryPlanner` filters named-break
`breakNeutralized` surfaces through the promoted exact named-break packet/proof
gate before planner-owned `WhyNow`, `WhatChanged`, `WhatMustBeStopped`, or
plan-race prose can surface. For this admission-unit pass that means
`neutralize_key_break`; the already-promoted `counterplay_restraint`
prophylactic lane keeps its own exact packet gate.

The same 2026-05-14 follow-up added route-level `BreakClampMaterializer`
evidence as a narrow runtime carrier materializer under
`ProphylaxisAnalyzerImpl`, before `BreakPreventionWitness` judges exactness. It
creates only existing `PreventedPlan` carriers on exact boards where a
null-turn opponent contact break route is legal before the move, illegal after
...
`max-candidates=24` and `admit-limit=1`. The triage report considered 24
ranked clean candidates and exposed exactly one promotable row under the pass
...
`counterplay_restraint`. Runtime release remains narrow: the exact
`counterplay_restraint` MoveDelta packet may downgrade only to
`SupportedLocal`, and only when the packet already proves the named prevented
...
`tmp/strategic_admission_unit_review.md`; it dry-runs transient source rows
through `SourceReview` and never changes runtime authority or the fixed source
catalog by itself. The first Modern Benoni 6.e4 run reviewed 24 ranked
transient rows and admitted `0` natural `prophylaxis_restraint` rows, with
blockers split as `owner:prophylaxis_restraint_witness_missing=18`,
`proof:prophylaxis_restraint_contract_mismatch=6`, and
`engine:source_move_absent_from_multipv=5`. This keeps the pass useful as
coverage tooling and blocker evidence without opening broad prophylaxis prose.

Failure taxonomy anchors for authority rows:

| matrix taxonomy | meaning |
| --- | --- |
| `certified_owner_path` | exact owner path releases; closed risk for overclaim / support-only reinflation |
| `weak_owner_path` | controlled weak-main degradation; allowed only as downgraded local reading |
...
| `owner:break_prevention_witness_missing` | controlled matrix umbrella for the missing exact named-break witness negative; SourceReview now expands natural-source misses into concrete `break_prevention_*` blockers |
| `owner:break_prevention_no_prevented_plan` | no current `PreventedPlanInfo` carrier is available, so source vocabulary cannot create authority |
| `owner:break_prevention_no_named_break` | a current prevented-plan carrier exists but does not name a measurable break with local counterplay loss |
| `owner:break_prevention_route_identity_missing` | a route-shaped named-break carrier cannot be matched back to a legal pre-move board route |
...
| `owner:break_prevention_branch_missing` | a named-break witness exists but lacks the best-defense branch key required for exact proof |
| `owner:break_prevention_persistence_unstable` | a named-break witness exists but same-family persistence is not stable enough for exact proof |
| `owner:break_prevention_rival_release` | a named-break witness is blocked by move-order sensitivity or release-risk evidence |
| `owner:break_prevention_tactical_first` | tactical truth outranks the strategic named-break reading |
| `proof:break_prevention_contract_mismatch` | a source-window row materializes a rival/non-break packet, such as incidental IQP, and may not be relabeled as `break_prevention` |
| `owner:break_prevention_rival_or_relabel` | named-break shell is blocked by release-risk or generic/rival counterplay relabel |
| `iqp_inducement_supported_local` | exact IQP witness releases only as `SupportedLocal` through `iqp_inducement_probe` |
| `position_probe_not_certified` | current-position probe shape exists but lacks the certified slice |
| `fen_only_owner_path_missing` | FEN-only screen candidate lacks PV / same-branch owner proof |
| `same_job_or_conversion_relabel_blocked` | simplification-looking line collapses into same-job or generic conversion relabel |
| `attacking_piece_trade_unowned` | attacker/defender trade idea lacks a planner-owned move-local pressure-reduction owner |
| `root_best_near_miss` | local idea exists but does not own the root-best branch |
| `rival_release` | file pressure, heavy-piece release, or rival story survives |
| `tactic_first_relief` / `tactical_truth_first` | tactical truth outranks the strategic release |
| `compensation_target_pressure_boundary` | compensation/initiative target pressure is not a clean B/C owner |
| `prophylaxis_absorption` / `file_entry_relabel_boundary` | row belongs to another existing lane and must not be stolen by B/C release |
