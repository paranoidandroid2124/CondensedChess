...
| --- | --- | --- | --- | --- | --- |
| `opening-tempo` | `U1` | `Object` | `OpeningDevelopmentRegime` | `phase_gate` release context only | legacy phase label leaves `U`; no honest exact witness id survives |
| `middlegame-positional` | `U1` | `Object` | `DistributedContactRegime` | `contested_sectors`, phase continuity only | payload geometry was never a standalone lower witness; row leaves `U` |
| `transition-liquidation` | `U1` | `Delta` | `TradeCompressionCorridor` | `TradeInvariant`, `MoveLocal`, `TransitionBridge` | transition meaning survives as delta-layer corridor, not as `U` witness |
| `endgame-race` | `U1` | `Object` | `EndgameRaceScaffold` | `promotion_race`, `PasserComplex` | race-trigger meaning leaves `U`; low-material context stays object-side |
| `king attack` | `U2` | `Object` | `AttackScaffold` | `S02`, `S03` | inventory row is projection alias only, not a `U` witness |
| `material gain` | `U2` | `Witness / U-attached` | `material gain` shell | `material_harvest`, `winning_endgame` | host-scoped objective shell only |
| `structural damage` | `U2` | `Witness / U-attached` | `structural damage` shell | lower structural-cause roots | host-scoped objective shell only |
| `space gain` | `U2` | `Witness / U-attached` | `structural_space_claim` | `closed center`, `fixed chain` | surviving attached runtime contract |
...
| `draw/hold` | `U2` | `Object` | `FortressHoldingShell` | `TradeInvariant`, `S22` | label remains projection wording; hold-side owner is object-level |
| `center` | `U3` | `Witness / U-attached` | `center` shell | `S05`, `S21` | neutral theater shell only |
| `kingside` | `U3` | `Witness / U-attached` | `kingside` shell | king-theater consumers above `U` | neutral theater shell only |
| `queenside` | `U3` | `Witness / U-attached` | `queenside` shell | wing-play consumers above `U` | neutral theater shell only |
...
| `whole-board` | `U3` | `Witness / U-attached` | `whole-board` shell | broad whole-board consumers above `U` | neutral theater shell only |
| `king shelter` | `U4` | `Object` | `KingSafetyShell` | root `king_shelter_hole`, king-safety certification | exact shelter fact remains in `R` |
...
| `closed center` | `U5` | `Witness / U-attached` | `closed center` host shell | `structural_space_claim` | central-structure host vocabulary only; no standalone exact witness survives |
| `fixed chain` | `U5` | `Witness / U-attached` | `fixed chain` host shell | `structural_space_claim` | host vocabulary only; chain-topology witness does not survive in `U` |
...
| `central tension` | `U7` | `Object` | `CentralContactFront` | `available_lever_trigger`, `open center`, `S05`, `S21` | contact meaning leaves `U`; object admission is deferred until helper freeze |
...
| `open line` | `U8` | `Witness / U-attached` | `open line` shell | `file_lane_state`, `rook_on_open_file_state`, `king_file_diagonal_entry_axis` | host-scoped transformation shell only |
...
| `simplify` | `U8` | `Delta` | `TradeInvariant` | `S19` | semantic owner is delta-layer; label stays projection wording |
| `create passer` | `U8` | `Witness / U-attached` | `create passer` shell | `candidate_passer`, `PasserComplex`, `S15` | host-scoped transformation shell only |
