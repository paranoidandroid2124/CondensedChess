...
| `key file/rank` | `U4` | `Projection` | `S09 open or semi-open file penetration`; `S25 horizontal rank access` | `file_lane_state`, `rank_corridor_state_seed`; broad `horizontal_rank_access` remains future vocabulary only | mixed label survives as projection alias only; file-side handoff stays on same-file `S09` lane-entry bundles, while current rank-side handoff is only the exact legal cross-wing `S25` seed plus matching rank-access evidence; no `U` witness or generic key-rank owner survives |
| `bad piece` | `U4` | `Projection` | `S17 worst-piece improvement / bad-piece exchange` | live same-piece support seeds from `StrategySupportSeedInventory.md`; current live liability anchor reads `loose_piece_target_state`, `pinned_piece`, and `trapped_piece`, while `duty_bound_defender` and `weak_outpost_square_state` remain strengthening support only | evaluative piece-quality label only; exact `S17` start-ready status is owned by `StrategyProjectionBoundaryMatrix.md`; live lower seeds do not release projection by themselves |
...
| `rook lift` | `U6` | `Projection` | `rook lift` projection alias | `RedeploymentRoute`, `AccessNetwork`, `AttackScaffold`, `S09`, narrow `S25` rank corridor only when exact-board legal | split projection consumers only; no single truth owner survives; current `S25` does not revive move-history rook-lift ownership and may admit only from `rank_corridor_state_seed` plus matching evidence |
...
| `exchange defender` | `U8` | `Projection` | `S24 tactical conversion of a prepared target` | `DefenderDependencyNetwork`, `TradeInvariant`; live prepared-target support seeds from `StrategySupportSeedInventory.md` | split upper family; label is projection alias only; exact `S24` start-ready status is owned by `StrategyProjectionBoundaryMatrix.md`; live lower seeds do not release projection by themselves |
...
| `improve worst piece` | `U8` | `Projection` | `S17 worst-piece improvement / bad-piece exchange` | live same-piece support seeds from `StrategySupportSeedInventory.md`; current live liability anchor reads `loose_piece_target_state`, `pinned_piece`, and `trapped_piece`, while `duty_bound_defender` and `weak_outpost_square_state` remain strengthening support only | broad improvement label remains projection alias; exact `S17` start-ready status is owned by `StrategyProjectionBoundaryMatrix.md`; live lower seeds do not release projection by themselves |
