...
final class CommentaryApi(
  ...
  private[commentary] final case class GameChronicleRuntimeArtifacts(
  ...
  private val GameArcProbeRequestsPerMoment = 1
  ...
  private def recordActiveThesisAgreement(
    ...
    lila.mon.commentary.commentary.thesisAgreement("active", observation.agreed).increment()
    updateCommentaryOpsGauges()
  ...
  def commentaryOpsSnapshot(limit: Int = 20): CommentaryOpsBoard.Snapshot =
    ...
    CommentaryOpsBoard.Snapshot(
      ...
      fullgame = CommentaryOpsBoard.FullGameMetrics(
        ...
        repairAttempts = fullGameSegmentRepairAttemptCount.get(),
        repairBypassed = fullGameSegmentRepairBypassedCount.get(),
        softRepairApplied = fullGameSegmentSoftRepairAppliedCount.get(),
        mergedRetrySkipped = fullGameMergedRetrySkippedCount.get(),
        invalidReasonCounts = fullGameInvalidReasonCount.view.mapValues(_.get()).toMap
      ),
  ...
  private def updateCommentaryOpsGauges(): Unit =
    ...
    lila.mon.commentary.commentary.metric("dossier_attach_rate", "active").update(activeDossierAttachRate)
    lila.mon.commentary.commentary.metric("dossier_compare_rate", "active").update(activeDossierCompareRate)
    lila.mon.commentary.commentary.metric("dossier_route_ref_rate", "active").update(activeDossierRouteRate)
    lila.mon.commentary.commentary.metric("dossier_reference_failure_rate", "active").update(activeDossierReferenceFailureRate)
    lila.mon.commentary.commentary.metric("route_redeploy_count", "active").update(routeRedeployCount)
    lila.mon.commentary.commentary.metric("route_move_ref_count", "active").update(routeMoveRefCount)
    lila.mon.commentary.commentary.metric("route_hidden_safety_count", "active").update(routeHiddenSafetyCount)
  ...
  private def effectiveCommentaryMode(
    ...
    val requested = normalizeCommentaryMode(requestedLevel)
    if tier == PlanTier.Pro && requested == CommentaryMode.Active && allowAiPolish then CommentaryMode.Active
    else CommentaryMode.Polish
  ...
  private def resolvedOpenAiActiveNoteRouteMeta(
      asyncTier: Boolean,
      planTier: String,
      commentaryMode: String
  ): ActiveNoteRouteMeta =
    val route = openAiClient.activeNoteRouteSummary(asyncTier, planTier, commentaryMode)
    ActiveNoteRouteMeta(
      provider = "openai",
  ...
  private def resolvedActiveNoteRouteMeta(
    ...
    providerConfig.activeNoteProviderResolved match
      case "openai" =>
        resolvedOpenAiActiveNoteRouteMeta(asyncTier, planTier, commentaryMode)
      case "gemini" =>
        val openAiFallback = Option.when(openAiClient.isEnabled)(resolvedOpenAiActiveNoteRouteMeta(asyncTier, planTier, commentaryMode))
        ActiveNoteRouteMeta(
          provider = "gemini",
          configuredModel = trimmedOption(geminiClient.activeModelName),
          fallbackModel = openAiFallback.flatMap(_.configuredModel),
  ...
  private def activeNoteCacheFingerprint(
    ...
    if planTier != PlanTier.Pro || commentaryMode != CommentaryMode.Active then "-"
    else
      val route = resolvedActiveNoteRouteMeta(asyncTier, planTier, commentaryMode)
      List(
        route.provider,
    ...
    if !isAiPolishEnabledForRequest(allowAiPolish) then None
      ...
      baseModel.map { model =>
        CommentaryCacheContext(
          ...
          activeNoteFingerprint = activeNoteCacheFingerprint(asyncTier, normalizedPlanTier, normalizedCommentaryMode)
        )
      }
  ...
  private def evaluateStrategyCoverage(
    ...
    val normalizedLevel = normalizeCommentaryMode(commentaryMode)
    val shouldEnforce = normalizedTier == PlanTier.Pro && normalizedLevel == CommentaryMode.Active
  ...
  private[commentary] final case class ActiveNoteFinalizationTrace(
      stage: String,
      primaryKind: Option[lila.commentary.model.authoring.AuthorQuestionKind] = None,
      sourceMode: Option[String] = None,
      rejectReason: Option[String] = None,
      hardReasons: List[String] = Nil,
      warningReasons: List[String] = Nil,
      composeDebug: Option[ActiveStrategicCoachingBriefBuilder.DeterministicComposeDebug] = None,
      noteCandidate: Option[String] = None,
      validatorPassed: Boolean = false
  ):
    def isAttached: Boolean = stage == CommentaryApi.ActiveFinalizationStage.Attached
    def noteBuilt: Boolean = noteCandidate.exists(_.trim.nonEmpty)
    def plannerApproved: Boolean =
      stage != CommentaryApi.ActiveFinalizationStage.NoPrimary &&
        stage != CommentaryApi.ActiveFinalizationStage.PreselectionBlocked &&
        stage != CommentaryApi.ActiveFinalizationStage.SupportGateBlocked
  ...
  private def validateActiveStrategicNote(
      ...
      routeRefs: List[ActiveStrategicRouteRef],
      moveRefs: List[ActiveStrategicMoveRef],
      plannerPrimaryKind: Option[lila.commentary.model.authoring.AuthorQuestionKind] = None,
    ...
    val strategyReasons =
      ...
      else evaluateStrategyCoverage(trimmed, strategyPack, PlanTier.Pro, CommentaryMode.Active).reasons
  ...
  private def normalizeActiveRejectReason(
      stage: String,
      hardReasons: List[String],
    ...
    val detailedReasons =
      hardReasons
        .map(_.trim)
        .filter(_.nonEmpty)
        .filterNot(reason => reason == "active_note_rule_failed" || reason == "active_note_rule_empty")
    if stage == CommentaryApi.ActiveFinalizationStage.Attached then None
    else if stage == CommentaryApi.ActiveFinalizationStage.NoPrimary then Some("no_planner_primary")
    else if stage == CommentaryApi.ActiveFinalizationStage.PreselectionBlocked then detailedReasons.headOption.orElse(Some("preselection_blocked"))
    else if stage == CommentaryApi.ActiveFinalizationStage.SupportGateBlocked then Some("visible_support_missing")
    else if stage == CommentaryApi.ActiveFinalizationStage.NoteEmpty || noteCandidate.forall(_.trim.isEmpty) then Some("note_body_empty")
    else if detailedReasons.contains("active_note_prior_phrase_reuse") then Some("validator_independence_fail")
    else if detailedReasons.contains("strategy_route_missing") then Some("missing_required_route")
    else if detailedReasons.contains("strategy_plan_missing") || detailedReasons.contains("forward_plan_missing") then
      Some("missing_required_plan")
    else if detailedReasons.contains("strategy_focus_missing") || detailedReasons.contains("opponent_or_trigger_missing") then
      Some("missing_required_focus")
    else if detailedReasons.contains("empty_polish") then Some("note_body_empty")
    else if detailedReasons.nonEmpty then Some("note_body_too_generic")
    else None

  private def finalizeActiveStrategicDecision(
      decision: ActiveStrategicNoteDecision,
      plannerPrimaryKind: Option[lila.commentary.model.authoring.AuthorQuestionKind]
  ): (Option[String], ActiveNoteFinalizationTrace) =
    val sanitized = decision.note.map(PlayerProseBoundary.sanitize).map(_.trim).filter(_.nonEmpty)
    val surfaceBoundaryEvaluation = sanitized.map(PlayerProseBoundary.validateSanitized)
    val validatedNote = surfaceBoundaryEvaluation.filter(_.isAccepted).map(_.text)
    val noteCandidate =
      decision.composeDebug.flatMap(_.result).orElse(decision.note).map(_.trim).filter(_.nonEmpty)
    val validatorPassed = decision.note.exists(_.trim.nonEmpty) && decision.hardReasons.isEmpty
    val stage =
      if validatedNote.nonEmpty then CommentaryApi.ActiveFinalizationStage.Attached
      else if decision.note.isEmpty then
        if decision.hardReasons.contains("active_note_rule_empty") then CommentaryApi.ActiveFinalizationStage.NoteEmpty
        else CommentaryApi.ActiveFinalizationStage.ValidatorFailed
      else CommentaryApi.ActiveFinalizationStage.SurfaceBoundaryFailed
    val hardReasons =
      if stage == CommentaryApi.ActiveFinalizationStage.SurfaceBoundaryFailed then
        (decision.hardReasons ++ surfaceBoundaryEvaluation.toList.flatMap(_.reasons)).distinct
      else decision.hardReasons.distinct
    val rejectReason = normalizeActiveRejectReason(stage, hardReasons, noteCandidate)
    (
      validatedNote,
  ...
  private def deterministicActiveStrategicNoteDecision(
    ...
    def validateRuleNote(note: String): ActiveStrategicNoteValidator.Result =
      validateActiveStrategicNote(
        ...
        plannerPrimary = Some(plannerSelection.primary)
      )
    ...
    def acceptedDecision(
        validation: ActiveStrategicNoteValidator.Result,
        warningReasons: List[String]
    ): ActiveStrategicNoteDecision =
      ActiveStrategicNoteDecision(
        note = Some(validation.text),
  ...
  private def maybeGenerateActiveStrategicNote(
    ...
    val normalizedPlanTier = normalizePlanTier(planTier)
    val normalizedCommentaryMode = normalizeCommentaryMode(commentaryMode)
    val routeMeta = resolvedActiveNoteRouteMeta(asyncTier, normalizedPlanTier, normalizedCommentaryMode)
    val activeEligible =
      normalizedPlanTier == PlanTier.Pro &&
        normalizedCommentaryMode == CommentaryMode.Active
    ...
    if !activeEligible then
      ...
      val openAiFallbackRouteMeta =
        Option.when(routeMeta.provider != "openai" && openAiClient.isEnabled)(
          resolvedOpenAiActiveNoteRouteMeta(asyncTier, normalizedPlanTier, normalizedCommentaryMode)
        )

      def observedModelsForOpenAi(
          meta: ActiveNoteRouteMeta,
          leadingModels: List[String],
          results: OpenAiPolishResult*
      ): List[String] =
  ...
  private def maybePolishGameChronicleArtifacts(
    ...
    if !isAiPolishEnabledForRequest(allowAiPolish) || providerConfig.isNone then
      maybeAttachActiveNotes(Future.successful(response.copy(sourceMode = "rule", model = None)))
    else
      val basePolishLevel = CommentaryMode.Polish
      val polishTargetPlies = selectFullGamePolishTargetPlies(response.moments)
      val introSlots =
        LiveNarrativeCompressionCore.openingSlotsFromCompressedProse(response.intro)
      val conclusionSlots =
        LiveNarrativeCompressionCore.practicalSlotsFromCompressedProse(response.conclusion)
      ...
      val basePolishFut =
        for
          ...
          polishedMoments <- Future.traverse(response.moments) { moment =>
            if polishTargetPlies.contains(moment.ply) then
              ...
              val momentSlots =
                LiveNarrativeCompressionCore.decisionSlotsFromCompressedProse(moment.narrative)
        ...
        yield
          ...
          val sourceMode =
            if sourceModes.contains("ai_polished") then "ai_polished"
  ...
  private def attachActiveStrategicNotes(
    ...
    val compensationParityPlies =
      response.moments
        .filter(moment => ActiveCompensationPolicy.selectionEligible(moment.strategyPack))
        .map(_.ply)
        .toSet
    val selectedByPly = response.moments.filter(_.strategicBranch).map(_.ply).toSet ++ compensationParityPlies
    ...
    Future.traverse(response.moments) { moment =>
      val truthContract = truthContractsByPly.get(moment.ply)
      ...
      val activePolicyEligible =
        CommentaryApi.activePlannerPolicyEligible(moment, primaryKind)
      val plannerSelectedActive =
        CommentaryApi.activePlannerSelectionEligible(
      ...
      val plannerPrimaryCanAttachWithoutVisibleSupport =
        plannerSelection.exists(selection =>
          CommentaryApi.activePlannerPrimaryCanAttachWithoutVisibleSupport(selection.primary.questionKind)
        )
  ...
  private[commentary] def traceActiveFinalization(
      moment: GameChronicleMoment,
      deltaBundle: lila.commentary.analysis.PlayerFacingMoveDeltaBundle,
      dossier: Option[ActiveBranchDossier],
      routeRefs: List[ActiveStrategicRouteRef],
      moveRefs: List[ActiveStrategicMoveRef],
    ...
    else if !CommentaryApi.activePlannerPolicyEligible(moment, primaryKind) then
      ActiveNoteFinalizationTrace(
        stage = CommentaryApi.ActiveFinalizationStage.PreselectionBlocked,
        primaryKind = primaryKind,
        sourceMode = Some("omitted"),
    ...
    else if !CommentaryApi.activePlannerSelectionEligible(strategicBranchSelected, primaryKind) then
      ActiveNoteFinalizationTrace(
        stage = CommentaryApi.ActiveFinalizationStage.PreselectionBlocked,
        primaryKind = primaryKind,
        sourceMode = Some("omitted"),
  ...
  private[commentary] def analyzeGameChronicleLocalArtifacts(
      pgn: String,
      evals: List[MoveEval],
      style: String = "book",
    ...
    val requestedLevel =
      ...
      then
        CommentaryMode.Active
      else normalizeCommentaryMode(commentaryMode)
    val evalMap = evals.map(e => e.ply -> e.getVariations).toMap
    val normalizedVariant = EarlyOpeningNarrationPolicy.normalizeVariantKey(variant)
    val normalizedProbeResultsByPly = probeResultsByPly.view.mapValues(_.take(GameArcProbeRequestsPerMoment)).toMap
    if normalizedProbeResultsByPly.nonEmpty then
      logger.info(
        s"[gamearc.probes] supplied_probe_moments=${normalizedProbeResultsByPly.size} supplied_probe_results=${normalizedProbeResultsByPly.values.map(_.size).sum}"
      )
    fetchOpeningRefsForPgn(pgn)
      .map { openingRefsByFen =>
        CommentaryEngine.generateGameArcDiagnostic(
          ...
          probeResultsByPly = normalizedProbeResultsByPly,
          variantKey = normalizedVariant
        )
      }
      ...
      .flatMap { diagnostic =>
        ...
        val base = GameChronicleResponse.fromGameArc(
          ...
          model = None,
          planTier = normalizedPlanTier,
          commentaryMode = requestedLevel
        )
        maybePolishGameChronicleArtifacts(
          response = base,
          truthContractsByPly = diagnostic.truthContractsByPly,
          allowAiPolish = allowAiPolish,
          lang = normalizeLang(lang),
          asyncTier = asyncTier,
          planTier = normalizedPlanTier,
  ...
  def submitGameChronicleAsync(
    ...
    analyzeGameChronicleLocal(
      ...
      commentaryMode = commentaryMode,
      variant = req.variant,
      probeResultsByPly = normalizeProbeResultsByPly(req.probeResultsByPly)
    ).map {
      case Some(result) =>
        updateAsyncJob(jobId): current =>
          current.copy(
  ...
  ): Map[Int, List[ProbeResult]] =
    entries
      .getOrElse(Nil)
      .collect {
        case entry if entry.ply > 0 && entry.results.nonEmpty =>
          entry.ply -> entry.results.take(GameArcProbeRequestsPerMoment)
      }
  ...
  def getCcaHistory(userId: String): Fu[List[lila.commentary.model.CollapseAnalysis]] =
...
object CommentaryApi:
  ...
  private[commentary] def activeCompensationNoteExpected(surface: StrategyPackSurface.Snapshot): Boolean =
    ActiveCompensationPolicy.noteExpected(surface)

  private[commentary] def activeCompensationNoteExpected(strategyPack: Option[StrategyPack]): Boolean =
    ActiveCompensationPolicy.noteExpected(strategyPack)

  private[commentary] def activeCompensationParityEligible(surface: StrategyPackSurface.Snapshot): Boolean =
    ActiveCompensationPolicy.parityEligible(surface)

  private[commentary] def activeCompensationParityEligible(strategyPack: Option[StrategyPack]): Boolean =
    ActiveCompensationPolicy.parityEligible(strategyPack)

  private[commentary] def activeCompensationSelectionEligible(surface: StrategyPackSurface.Snapshot): Boolean =
    ActiveCompensationPolicy.selectionEligible(surface)

  private[commentary] def activeCompensationSelectionEligible(strategyPack: Option[StrategyPack]): Boolean =
    ActiveCompensationPolicy.selectionEligible(strategyPack)

  private[commentary] object ActiveFinalizationStage:
    ...
    val SurfaceBoundaryFailed = "surface_boundary_failed"
    val Attached = "attached"

  private[commentary] def activePlannerSelectionEligible(
      strategicBranchSelected: Boolean,
      primaryKind: Option[lila.commentary.model.authoring.AuthorQuestionKind]
  ): Boolean =
    strategicBranchSelected || primaryKind.contains(lila.commentary.model.authoring.AuthorQuestionKind.WhyNow)

  private[commentary] def activePlannerPolicyEligible(
      moment: GameChronicleMoment,
      primaryKind: Option[lila.commentary.model.authoring.AuthorQuestionKind]
  ): Boolean =
    PlayerFacingTruthModePolicy.allowsActiveNote(moment) ||
      primaryKind.contains(lila.commentary.model.authoring.AuthorQuestionKind.WhyNow)

  private[commentary] def activePlannerPrimaryCanAttachWithoutVisibleSupport(
      kind: lila.commentary.model.authoring.AuthorQuestionKind
  ): Boolean =
    kind == lila.commentary.model.authoring.AuthorQuestionKind.WhyNow
