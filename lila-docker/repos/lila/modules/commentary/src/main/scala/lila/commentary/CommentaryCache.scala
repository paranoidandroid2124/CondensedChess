...
final class CommentaryCache(using Executor):
  ...
  def get(
    ...
    cache.getIfPresent(cacheKey(fen, lastMove, probeResults, planStateToken, endgameStateToken, commentaryContext))
  ...
  def put(
    ...
    cache.put(cacheKey(fen, lastMove, probeResults, planStateToken, endgameStateToken, commentaryContext), response)
  ...
  def invalidate(
    ...
    cache.invalidate(cacheKey(fen, lastMove, probeResults, planStateToken, endgameStateToken, commentaryContext))
