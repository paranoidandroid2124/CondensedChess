package lila.commentary.tools.realpgn
...
import lila.commentary.analysis.{ CommentaryEngine, CompensationContractMatcher, CompensationRecaptureBoundary, EarlyOpeningNarrationPolicy, NarrativeUtils, OpeningExplorerClient, StrategyPackSurface }
import lila.commentary.model.{ FutureSnapshot, L1DeltaSnapshot, ProbeRequest, ProbeResult, TargetsDelta }
import lila.commentary.model.strategic.VariationLine
import lila.commentary.tools.review.CommentaryPlayerQcSupport

object RealPgnNarrativeEvalRunner:

  private val DefaultCorpusPath = Paths.get("modules/commentary/docs/RealPgnNarrativeEvalCorpus_20260316.json")
  private val DefaultMarkdownPath = Paths.get("modules/commentary/docs/RealPgnNarrativeEvalReport.latest.md")
  private val DefaultJsonPath = Paths.get("modules/commentary/docs/RealPgnNarrativeEvalReport.latest.json")
  private val DefaultRawDir = Paths.get("modules/commentary/docs/real_pgn_eval/latest")
  private val DefaultDepth = 10
  private val DefaultMultiPv = 3
  private val EngineEnvVars = List("STOCKFISH_BIN", "AI_ACTIVE_CORPUS_ENGINE_PATH")
  ...
  private final case class ProbeMomentBundle(
      ply: Int,
      requests: List[lila.commentary.model.ProbeRequest]
  )
  ...
  private def probeDedupKey(request: lila.commentary.model.ProbeRequest): String =
    List(
      request.fen,
  ...
  private[commentary] final class LocalUciEngine(enginePath: Path, timeoutMs: Long):
    private val process =
