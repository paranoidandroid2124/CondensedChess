...
class NamedRouteChainBindProofBoundaryTest extends FunSuite:
  ...
  private def contractFor(
      ...
      PlanEvidenceEvaluator.EvaluatedPlan,
      LocalFileEntryProof.Contract,
      RouteNetworkBindProof.Contract
  ) =
  ...
  private def plannerInputs(
      ...
      namedRouteNetworkSurface: Option[RouteNetworkBindProof.SurfaceNetwork],
  ...
  test("raw route-chain helper may keep the exact chain backend-only while planner wording stays closed") {
    ...
    val rawSurface =
      RouteNetworkBindProof.certifiedSurfaceNetwork(preventedPlansNow, List(broaderPlan, exactPlan))
