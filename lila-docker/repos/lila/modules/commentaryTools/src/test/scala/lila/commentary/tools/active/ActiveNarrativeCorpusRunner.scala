package lila.commentary.tools.active
...
import lila.commentary.analysis.{ CommentaryOpsBoard, OpeningExplorerClient }

object ActiveNarrativeCorpusRunner:

  import ActiveNarrativeCorpusSupport.*

  private val DefaultCorpusPath = Paths.get("modules/commentary/docs/ActiveNarrativePlayerCorpus_20260311.json")
  private val DefaultEvalCachePath = Paths.get("modules/commentary/docs/ActiveNarrativePlayerCorpus_20260311.evals.json")
  private val DefaultReportMarkdownPath = Paths.get("modules/commentary/docs/ActiveNarrativeCorpusReport.latest.md")
  private val DefaultReportJsonPath = Paths.get("modules/commentary/docs/ActiveNarrativeCorpusReport.latest.json")
  private val DefaultRawResponseDir = Paths.get("modules/commentary/docs/active_narrative_runs/latest/responses")
  ...
  def main(args: Array[String]): Unit =
    ...
    val providerConfig = AiProviderConfig.fromEnv
    if providerConfig.isNone then
      System.err.println("[active-corpus-runner] `AI_PROVIDER` resolved to `none`; cannot run Pro+Active benchmark.")
      sys.exit(1)
      ...
      .map { reports =>
        val runReport =
          RunReport(
            ...
            commentaryMode = config.commentaryMode,
    ...
    override def analyze(entry: CorpusEntry, evals: List[MoveEval]): Future[AnalysisArtifacts] =
      for
        ...
        responseOpt <- api.analyzeGameChronicleLocal(
          ...
          allowAiPolish = config.allowAiPolish,
