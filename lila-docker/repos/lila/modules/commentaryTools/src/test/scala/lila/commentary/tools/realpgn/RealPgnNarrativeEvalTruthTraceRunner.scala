...
import lila.commentary.analysis.{ CommentaryEngine, EarlyOpeningNarrationPolicy, NarrativeUtils, OpeningExplorerClient, StrategyPackSurface }
import lila.commentary.model.OpeningReference
import lila.commentary.model.{ FutureSnapshot, L1DeltaSnapshot, TargetsDelta }
...
object RealPgnNarrativeEvalTruthTraceRunner:
  ...
  private val EngineEnvVars = List("AI_ACTIVE_CORPUS_ENGINE_PATH", "STOCKFISH_BIN")
  ...
  private def fetchOpeningRefsForPgn(api: CommentaryApi, pgn: String)(using Executor): Map[String, OpeningReference] =
    val openingFens =
  ...
  private def executeSupportedProbeRequests(
      ...
      engine: CommentaryPlayerQcSupport.LocalUciEngine
  ): (Map[Int, List[lila.commentary.model.ProbeResult]], Int, Int) =
    val supportedSignals = Set("replyPvs", "keyMotifs", "l1Delta", "futureSnapshot")
    var unsupported = 0
    val byPly = scala.collection.mutable.Map.empty[Int, List[lila.commentary.model.ProbeResult]].withDefaultValue(Nil)
      ...
      bundle.requests.foreach { request =>
        ...
        if required.subsetOf(supportedSignals) then
          ...
          vars.headOption.foreach { best =>
            ...
            val result =
              lila.commentary.model.ProbeResult(
                id = request.id,
  ...
  private def synthesizeKeyMotifs(
      request: lila.commentary.model.ProbeRequest,
      best: VariationLine,
      moverLoss: Int
  ): List[String] =
    val raw =
  ...
  private def planProgressHints(request: lila.commentary.model.ProbeRequest, keyMotifs: List[String]): List[String] =
    val raw =
