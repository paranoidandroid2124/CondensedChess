...
object ActiveNarrativeCorpusSupport:
  ...
  final case class CorpusEntry(
    ...
    lazy val totalPlies: Int =
      lila.commentary.PgnAnalysisHelper.extractPlyData(game.pgn).map(_.size).getOrElse(0)
  ...
  def renderMarkdown(report: RunReport): String =
    ...
    lines += s"- Mode: `${report.planTier}/${report.commentaryMode}`"
