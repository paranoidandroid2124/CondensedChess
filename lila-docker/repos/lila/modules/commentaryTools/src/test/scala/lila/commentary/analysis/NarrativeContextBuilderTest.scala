...
class NarrativeContextBuilderTest extends FunSuite {
  ...
  def minimalData(ctx: Option[IntegratedContext] = None): ExtendedAnalysisData = 
    ExtendedAnalysisData(
      fen = testFen,
      nature = PositionNature(lila.commentary.model.NatureType.Dynamic, 0.5, 0.5, "Dynamic position"),
      motifs = Nil,
      plans = Nil,
      preventedPlans = Nil,
      pieceActivity = Nil,
  ...
  test("summary key threat uses relation fallback labels instead of raw motif names") {
    val threat = Threat(
  ...
  test("A3: buildHeader populates from IntegratedContext.classification") {
    val classification = PositionClassification(
      nature = NatureResult(lila.commentary.analysis.L3.NatureType.Dynamic, 3, 2, 1, false),
  ...
  test("A7: buildCandidates converts UCI to SAN") {
    val candidateLine = lila.commentary.model.strategic.VariationLine(
  ...
  test("Probe: emits LEGAL UCI moves when a high-score plan is not represented in top MultiPV moves") {
    ...
    val pv1 = lila.commentary.model.strategic.VariationLine(
      moves = List("e2e4", "e7e5"),
      scoreCp = 20,
      mate = None,
      tags = Nil
  ...
  test("Probe: score threshold prevents noisy probes") {
    ...
    val pv1 = lila.commentary.model.strategic.VariationLine(moves = List("e2e4"), scoreCp = 0, mate = None, tags = Nil)
  ...
  test("B1: ChoiceType derived from classification.choiceTopology (OnlyMove)") {
    val classification = PositionClassification(
      nature = NatureResult(lila.commentary.analysis.L3.NatureType.Dynamic, 2, 1, 0, false),
  ...
  test("B3: DivergePly populated when counterfactual exists with punisher") {
    val userLine = lila.commentary.model.strategic.VariationLine(
    ...
    val counterfactual = lila.commentary.model.strategic.CounterfactualMatch(
      userMove = "e2e4",
      bestMove = "d2d4",
      cpLoss = 50,
      missedMotifs = Nil,
  ...
  test("F1: buildPVDelta returns empty when bestProbe is None") {
    val ctx = IntegratedContext(
      ...
      classification = Some(PositionClassification(
        nature = NatureResult(lila.commentary.analysis.L3.NatureType.Dynamic, 0, 0, 0, false),
