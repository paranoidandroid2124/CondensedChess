...
import lila.commentary.model.authoring.AuthorQuestion
import lila.commentary.model.authoring.AuthorQuestionKind
import lila.commentary.model.strategic.VariationLine

object TaskShiftSourceReview:

  private val fixtures = TaskShiftProvingFixtures.reviewFixtures

  @main def runTaskShiftSourceReview(ids: String*): Unit =
  ...
  private def renderPlans(plans: List[lila.commentary.model.authoring.PlanHypothesis]): String =
  ...
  private def renderIdeas(pack: lila.commentary.StrategyPack): String =
  ...
  private def renderTargetFixationDiag(
      ...
      packOpt: Option[lila.commentary.StrategyPack]
  ): String =
    val targetFixIdea =
      packOpt.toList
        .flatMap(_.strategicIdeas)
        .find(_.kind == lila.commentary.StrategicIdeaKind.TargetFixing)
    val branchKey =
  ...
  private def renderTargets(pack: lila.commentary.StrategyPack): String =
  ...
  private def renderMoveRefs(pack: lila.commentary.StrategyPack): String =
  ...
  private def renderRoutes(pack: lila.commentary.StrategyPack): String =
  ...
  private def renderPlanMatches(plans: List[lila.commentary.model.PlanMatch]): String =
    if plans.isEmpty then "-"
    else
      plans
        .take(5)
