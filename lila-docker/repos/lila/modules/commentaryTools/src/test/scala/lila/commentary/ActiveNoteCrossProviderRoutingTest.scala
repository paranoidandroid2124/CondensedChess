...
class ActiveNoteCrossProviderRoutingTest extends FunSuite:
  ...
  private def api(openAiEnabled: Boolean): CommentaryApi =
  ...
  private def resolvedRouteMeta(api: CommentaryApi): Product =
    val m = classOf[CommentaryApi].getDeclaredMethod(
    ...
    m.invoke(api, Boolean.box(false), PlanTier.Pro, CommentaryMode.Active).asInstanceOf[Product]
