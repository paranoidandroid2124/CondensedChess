package lila.commentary.tools.strategicpuzzle
...
import lila.commentary.analysis.{ CommentaryEngine, NarrativeContextBuilder, OpeningExplorerClient, StrategyPackBuilder }
import lila.commentary.model.NarrativeRenderMode
import lila.commentary.model.strategic.VariationLine
import lila.commentary.tools.strategicpuzzle.StrategicPuzzleCorpusSupport.*
...
object StrategicPuzzleCorpusRunner:
  ...
  private def run(config: Config, ws: StandaloneAhcWSClient)(using Executor): Unit =
    ...
    val preAiPath = config.outDir.resolve("stage02_pre_ai_survivors.jsonl")
      ...
      val report =
        Json.obj(
          ...
          "counts" -> Json.obj(
            ...
            "preAiSurvivors" -> stage02Rows.size,
            "rejectRows" -> rejectRows.size
          )
        )
      writeJsonl(rejectPath, rejectRows.distinctBy(rr => s"${rr.seedId}|${rr.stage}|${rr.reasons.mkString(",")}"))
      writeJson(reportPath, report)
      println(s"[strategic-puzzle] stop-before-ai mode wrote ${stage02Rows.size} survivors to $preAiPath")
      return
  ...
  private def mkApi(ws: StandaloneAhcWSClient)(using Executor): CommentaryApi =
    CommentaryApi(
      ...
      providerConfig = AiProviderConfig.fromEnv.copy(polishGateThreshold = 2.0, premiumOnly = false)
    )
  ...
  private def parseArgs(args: List[String]): Either[String, Config] =
    ...
    def loop(rest: List[String], cfg: Config): Either[String, Config] =
      rest match
        ...
        case "--stop-before-ai" :: tail => loop(tail, cfg.copy(stopBeforeAi = true))
