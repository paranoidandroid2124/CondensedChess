...
object StrategicPuzzlePrePolishRunner:
  ...
  private def run(config: Config, ws: StandaloneAhcWSClient)(using Executor): Unit =
    ...
    println(s"[strategic-prepolish] loaded ${stage02Rows.size} pre-AI survivors from ${config.inputPath}")
  ...
  private def detectDefaultInput(workspaceRoot: Path): Path =
    ...
    val default = runsDir.resolve(Path.of("preai_full_20260318", "stage02_pre_ai_survivors.jsonl"))
    if Files.exists(default) then default
      ...
      Option.when(Files.isDirectory(runsDir)) {
        ...
        .filter(path => Files.isDirectory(path) && path.getFileName.toString.startsWith("preai_full_"))
        .sortBy(_.getFileName.toString)
        .reverse
        .map(_.resolve("stage02_pre_ai_survivors.jsonl"))
        .find(Files.exists(_))
        .getOrElse(default)
