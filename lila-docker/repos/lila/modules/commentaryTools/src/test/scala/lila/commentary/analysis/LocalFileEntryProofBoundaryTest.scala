...
class LocalFileEntryProofBoundaryTest extends FunSuite:
  ...
  test("deferred experiment plus residual main plan cannot stitch a file-entry claim back into builder or planner surfaces") {
    ...
    assertEquals(LocalFileEntryProof.certifiedSurfacePair(ctx), None, clues(ctx.mainStrategicPlans, ctx.strategicPlanExperiments))
