...
class StrategicPuzzleCorpusSupportTest extends FunSuite:
  ...
  test("explanation gate enforces ai polished mode, length, signal richness, and strategy coverage") {
    val failing =
      mkCommentResponse(
        dominantIdeaKind = Some("line_occupation"),
        directionalTarget = Some("e5"),
    ...
    assert(failedGate.reasons.contains("source_mode_not_ai_polished"))
    ...
    assertEquals(passedGate.sourceMode, "ai_polished")
  ...
  test("pre-polish gate allows rule-mode strategy signals to qualify for family clustering") {
    ...
    assert(!gate.reasons.contains("source_mode_not_ai_polished"))
    assert(family.robust, clue(family))
