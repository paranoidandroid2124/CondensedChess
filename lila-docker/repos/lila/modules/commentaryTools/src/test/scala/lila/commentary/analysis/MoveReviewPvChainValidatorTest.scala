...
final class MoveReviewPvChainValidatorTest extends FunSuite:
  ...
  test("validates a legal PV chain against every recorded fenAfter") {
    ...
    val validated = MoveReviewPvChainValidator.validatedLine(fen, pv, "f1c4")
  ...
  test("rejects a PV chain when a recorded fenAfter does not match legal replay") {
    val fen = "r1bqkbnr/pppp1ppp/2n5/4p3/4P3/5N2/PPPP1PPP/RNBQKB1R w KQkq - 2 3"
    val pv = line(fen, List("f1c4", "g8f6", "d2d3"), List("Bc4", "Nf6", "d3"))
    val corrupted =
      pv.copy(moves = pv.moves.updated(1, pv.moves(1).copy(fenAfter = pv.moves.head.fenAfter)))

    assertEquals(MoveReviewPvChainValidator.validatedLine(fen, corrupted, "f1c4"), None)
  }

  test("rejects illegal UCI and wrong side-to-move") {
    val fen = "r1bqkbnr/pppp1ppp/2n5/4p3/4P3/5N2/PPPP1PPP/RNBQKB1R w KQkq - 2 3"
    val illegal = line(fen, List("f1c4", "g8g6"), List("Bc4", "Ng6"))
    val wrongSide = line(fen, List("g8f6", "f1c4"), List("Nf6", "Bc4"))

    assertEquals(MoveReviewPvChainValidator.validatedLine(fen, illegal, "f1c4"), None)
    assertEquals(MoveReviewPvChainValidator.validatedLine(fen, wrongSide, "g8f6"), None)
  }

  test("rejects missing fenAfter, unordered ply, and mismatched start FEN") {
    val fen = "r1bqkbnr/pppp1ppp/2n5/4p3/4P3/5N2/PPPP1PPP/RNBQKB1R w KQkq - 2 3"
    val pv = line(fen, List("f1c4", "g8f6", "d2d3"), List("Bc4", "Nf6", "d3"))
    ...
    assertEquals(MoveReviewPvChainValidator.validatedLine(fen, missingFen, "f1c4"), None)
    assertEquals(MoveReviewPvChainValidator.validatedLine(fen, unordered, "f1c4"), None)
    assertEquals(MoveReviewPvChainValidator.validatedLine(otherStart, pv, "f1c4"), None)
  }
  ...
  test("validates castling, promotion, and en-passant legal replay") {
    ...
    assert(MoveReviewPvChainValidator.validatedLine(castleFen, castle, "e1g1").nonEmpty)

    val promotionFen = "4k3/P7/8/8/8/8/8/4K3 w - - 0 1"
    val promotion = line(promotionFen, List("a7a8q", "e8f7"), List("a8=Q", "Kf7"), "promotion")
    assert(MoveReviewPvChainValidator.validatedLine(promotionFen, promotion, "a7a8q").nonEmpty)
    ...
    assert(MoveReviewPvChainValidator.validatedLine(epFen, enPassant, "e5d6").nonEmpty)
  }
