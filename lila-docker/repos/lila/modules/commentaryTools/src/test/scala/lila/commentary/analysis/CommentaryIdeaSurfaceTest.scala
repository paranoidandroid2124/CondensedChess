...
final class CommentaryIdeaSurfaceTest extends FunSuite:
  ...
  test("normal development descriptors are pinned to exact legal opening PVs") {
    ...
    assertEquals(italian.opponentReplyMeaning, None, clue(italian))
    ...
    assertEquals(ruy.opponentReplyMeaning, None, clue(ruy))
  ...
  test("PV-backed tactical direct-threat and endgame descriptors keep current admission gates") {
    ...
    assertEquals(tactical.opponentQuestion, None, clue(tactical.opponentQuestion))
    assertEquals(tactical.lineResolution, None, clue(tactical.lineResolution))
    ...
    assertEquals(directThreat.opponentQuestion, None, clue(directThreat.opponentQuestion))
