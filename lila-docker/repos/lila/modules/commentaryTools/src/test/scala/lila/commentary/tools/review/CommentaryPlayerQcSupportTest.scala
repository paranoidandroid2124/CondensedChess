...
class CommentaryPlayerQcSupportTest extends FunSuite:
  ...
  private def minimalExtendedData(ply: Int): lila.commentary.model.ExtendedAnalysisData =
    lila.commentary.model.ExtendedAnalysisData(
      fen = questionFen,
      nature = lila.commentary.model.PositionNature(lila.commentary.model.NatureType.Dynamic, 0.5, 0.5, "Dynamic"),
      motifs = Nil,
      plans = Nil,
      preventedPlans = Nil,
      pieceActivity = Nil,
      ...
      isWhiteToMove = true,
      phase = "middlegame",
      integratedContext = Some(lila.commentary.analysis.IntegratedContext(evalCp = 35, isWhiteToMove = true))
    )
  ...
  private def whyNowSnapshot(gameKey: String, ply: Int, concreteOwner: Boolean): SliceSnapshot =
    ...
    val ctx =
      lila.commentary.model.NarrativeContext(
        fen = questionFen,
        header = lila.commentary.model.ContextHeader("Middlegame", "Normal", "NarrowChoice", "Medium", "ExplainPlan"),
        ply = ply,
        playedMove = Some("e2e4"),
        playedSan = Some("e4"),
        summary = lila.commentary.model.NarrativeSummary("Central pressure", None, "NarrowChoice", "Maintain", "0.00"),
        threats =
          lila.commentary.model.ThreatTable(
            toUs =
              Option.when(concreteOwner)(
                List(
                  lila.commentary.model.ThreatRow(
                    kind = "Mate",
                    side = "US",
                    square = Some("h7"),
                    lossIfIgnoredCp = 500,
        ...
        pawnPlay = lila.commentary.model.PawnPlayTable(false, None, "Low", "Maintain", "Quiet", "Background", None, false, "quiet"),
        plans = lila.commentary.model.PlanTable(Nil, Nil),
        delta = None,
        phase = lila.commentary.model.PhaseContext("Middlegame", "Balanced middlegame"),
        candidates = Nil,
        authorQuestions = List(question),
        authorEvidence = List(whyNowEvidence(question.id)),
        renderMode = lila.commentary.model.NarrativeRenderMode.FullGame
      )
    SliceSnapshot(
      entry =
        CatalogEntry(
    ...
    val candidate =
      base.copy(
        ctx =
          base.ctx.copy(
            openingEvent =
              Some(lila.commentary.model.OpeningEvent.OutOfBook("Qc2", List("Re1", "a3", "h3"), 24))
          )
      )
            ...
            openingData =
              Some(
                lila.commentary.model.OpeningReference(
                  ...
                  topMoves =
                    List(
                      lila.commentary.model.ExplorerMove(
                        uci = "c2c4",
                        san = "c4",
                        total = 240,
                        white = 92,
