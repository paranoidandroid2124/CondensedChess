...
object CommentaryPlayerRerunShardBuilder:
  ...
  import lila.commentary.tools.realpgn.RealPgnNarrativeEvalRunner.{ Corpus, CorpusGame }
...
try {
      sbt 'commentaryTools/test:runMain lila.commentary.tools.realpgn.RealPgnNarrativeEvalReportMerge ${quoteSbtArg(reportRoot)} ${quoteSbtArg(mergedMd)} ${quoteSbtArg(mergedJson)}'
} finally {
  Pop-Location
}
"""
  ...
  private def renderScript(shards: List[ShardMeta], header: String)(commandFor: ShardMeta => String): String =
    ...
    s"""Set-StrictMode -Version Latest
# $header
if (-not $$env:STOCKFISH_BIN -and -not $$env:AI_ACTIVE_CORPUS_ENGINE_PATH) {
  throw "Set STOCKFISH_BIN or AI_ACTIVE_CORPUS_ENGINE_PATH before running shard jobs."
}
Push-Location ${quoteLiteral(repoRoot().toString)}
try {
$body} finally {
