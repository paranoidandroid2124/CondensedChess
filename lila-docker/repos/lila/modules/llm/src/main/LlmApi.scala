...
final class LlmApi(
  ...
  private def resolvedActiveNoteRouteMeta(
    ...
    providerConfig.activeNoteProviderResolved match
      case "openai" =>
        resolvedOpenAiActiveNoteRouteMeta(asyncTier, planTier, llmLevel)
      case "gemini" =>
        val openAiFallback = Option.when(openAiClient.isEnabled)(resolvedOpenAiActiveNoteRouteMeta(asyncTier, planTier, llmLevel))
        ActiveNoteRouteMeta(
          provider = "gemini",
  ...
  private def normalizeActiveRejectReason(
    ...
    if stage == LlmApi.ActiveFinalizationStage.Attached then None
    else if stage == LlmApi.ActiveFinalizationStage.NoPrimary then Some("no_planner_primary")
    else if stage == LlmApi.ActiveFinalizationStage.PreselectionBlocked then detailedReasons.headOption.orElse(Some("preselection_blocked"))
    else if stage == LlmApi.ActiveFinalizationStage.SupportGateBlocked then Some("visible_support_missing")
    else if stage == LlmApi.ActiveFinalizationStage.NoteEmpty || noteCandidate.forall(_.trim.isEmpty) then Some("note_body_empty")
    ...
    if !activeEligible then
      ...
      val openAiFallbackRouteMeta =
        Option.when(routeMeta.provider != "openai" && openAiClient.isEnabled)(
          resolvedOpenAiActiveNoteRouteMeta(asyncTier, normalizedPlanTier, normalizedLlmLevel)
        )
        ...
        rewritesFut.map { results =>
          ...
          if successes.isEmpty then None
            ...
            if mergedReasons.nonEmpty then
              ...
              val repairedStrategyValidation = evaluateStrategyCoverage(repairedText, strategyPack, planTier, llmLevel)
  ...
  private def attachActiveStrategicNotes(
    ...
    val compensationParityPlies =
      response.moments
        .filter(moment => LlmApi.activeCompensationSelectionEligible(moment.strategyPack))
        .map(_.ply)
        .toSet
    ...
    commentaryCache.get(fen, lastMove, incomingProbes, prevStateToken, prevEndgameStateToken, cacheCtx) match
      ...
      case None =>
        ...
        computeBookmakerResponse(
          ...
          afterFen, afterEval, afterVariations, opening, phase, ply, variant, prevStateToken, prevEndgameStateToken, allowLlmPolish, lang, cacheCtx, normalizedPlanTier, effectiveLevel
        ).map:
...
private[llm] object RuleTemplateSanitizer:
  ...
  def sanitize(text: String, opening: Option[String], phase: String, ply: Int, fen: Option[String] = None): String =
    ...
    lila.llm.analysis.UserFacingSignalSanitizer.sanitize(
...
object LlmApi:
  ...
  private[llm] def activeCompensationNoteExpected(strategyPack: Option[StrategyPack]): Boolean =
    activeCompensationNoteExpected(StrategyPackSurface.from(strategyPack))

  private[llm] def activeCompensationParityEligible(surface: StrategyPackSurface.Snapshot): Boolean =
  ...
  private[llm] def activeCompensationParityEligible(strategyPack: Option[StrategyPack]): Boolean =
    activeCompensationParityEligible(StrategyPackSurface.from(strategyPack))

  private[llm] def activeCompensationSelectionEligible(surface: StrategyPackSurface.Snapshot): Boolean =
    ...
    activeCompensationParityEligible(surface) ||
      (activeCompensationNoteExpected(surface) && hasConcreteCarrierAnchor)
