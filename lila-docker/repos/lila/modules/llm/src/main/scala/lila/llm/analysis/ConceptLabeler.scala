...
enum EndgameTag:
  ...
  case Zugzwang
  case PawnRace(winner: Option[Color])
  case OutsidePassedPawn(side: Color)
...
enum PositionalTag:
  ...
  case LoosePiece(square: String, role: Role, side: Color)
  ...
  def toSnakeCase: String = this match
    ...
    case LoosePiece(sq, _, c) => s"${c.name.toLowerCase}_loose_piece_$sq"
...
object ConceptLabeler:
  ...
  def labelPosition(
    ...
    val positional = labelPositional(board, color, features, motifs)
  ...
  private def labelEndgame(
    ...
    if phase == "endgame" then
      ...
      // Detect Outside Passed Pawn
      detectOutsidePassedPawn(features.pawns).foreach(color => tags += EndgameTag.OutsidePassedPawn(color))

      // Detect Pawn Race
      val colorToMove = chess.Color.fromName(features.sideToMove).getOrElse(chess.Color.White)
      detectPawnRace(features.pawns, colorToMove).foreach(winner => tags += EndgameTag.PawnRace(winner))

      // Detect Wrong Bishop Draw (Basic heuristic: winning material but drawish eval)
      if (Math.abs(bestEval) < 50 && Math.abs(features.materialPhase.materialDiff) > 2) then
        // If strict wrong bishop logic needed, would require board access here. 
        // For now, eval heuristic + material imbalance is a decent proxy.
        // Or we can rely on verifying 'draw by insufficient material' or tablebase if available.
        // Let's add a placeholder heuristic for now.
        tags += EndgameTag.WrongBishopDraw 

    tags.result().distinct

  private def detectOutsidePassedPawn(pawns: PawnStructureFeatures): Option[Color] =
    // Heuristic: If one side has a passed pawn significantly far from kings
    // This requires board access usually, but utilizing L1 features:
    if pawns.whitePassedPawns > 0 && pawns.blackPassedPawns == 0 then Some(Color.White)
    else if pawns.blackPassedPawns > 0 && pawns.whitePassedPawns == 0 then Some(Color.Black)
    else None

  private def detectPawnRace(pawns: PawnStructureFeatures, sideToMove: Color): Option[Option[Color]] =
    // Heuristic: Both sides have passed pawns and low piece count
    if pawns.whitePassedPawns > 0 && pawns.blackPassedPawns > 0 then
       // Ideally we'd calculate race distance. For now, tag the race existence.
  ...
  def labelPositional(
    ...
    detectLoosePieces(board, oppColor).take(2).foreach { sq =>
      val role = board.roleAt(sq).getOrElse(Piece(oppColor, Pawn).role)
      tags += PositionalTag.LoosePiece(sq.key, role, oppColor)
    }
