...
import lila.llm.model.strategic.{ VariationTag, CounterfactualMatch, Hypothesis }
...
object NarrativeGenerator:
  ...
  def describeExtended(data: ExtendedAnalysisData, style: NarrativeTemplates.Style = NarrativeTemplates.Style.Book): String =
    val parts = List.newBuilder[String]
    ...
    val seed = NarrativeTemplates.mixSeed(data.nature.description.hashCode.abs, data.ply, planHash)
  
    // FALLBACK for scant data
    if (data.motifs.isEmpty && data.plans.isEmpty) {
        parts += NarrativeTemplates.fallbackNature(natureType, data.nature.tension, seed)
    } else {
        parts += NarrativeTemplates.intro(natureType, data.nature.tension, seed, style)
    }
    ...
    if (data.structuralWeaknesses.nonEmpty) {
      ...
      val header = style match {
        case NarrativeTemplates.Style.Coach => "STRUCTURE (Study Point):"
        case NarrativeTemplates.Style.Dramatic => "STRUCTURAL BATTLEGROUND:"
        case _ => "STRUCTURE:"
      }
    ...
    if (data.positionalFeatures.nonEmpty) {
      ...
      val filteredFeatures = data.positionalFeatures.filter { 
        case lila.llm.model.strategic.PositionalTag.LoosePiece(sq, _) => 
      ...
      val posDescriptionsList = filteredFeatures.take(6).map {
        ...
        case lila.llm.model.strategic.PositionalTag.LoosePiece(sq, color) =>
      ...
      val header = style match {
        ...
        case NarrativeTemplates.Style.Coach => "KEY STRATEGIC FACTORS:"
        case NarrativeTemplates.Style.Dramatic => "THE BATTLEFIELD:"
    ...
    if (data.motifs.nonEmpty) {
      ...
      val tacticsHeader = style match {
        case NarrativeTemplates.Style.Coach => "TACTICS (Study these patterns):"
        case NarrativeTemplates.Style.Dramatic => "TACTICAL FIREWORKS:"
        case _ => "TACTICS:"
      }
      parts += s"$tacticsHeader $motifDescriptions"
      
      // Issue #4 Fix: Rhetorical question for tactics (Coach/Dramatic)
      if (style == NarrativeTemplates.Style.Coach || style == NarrativeTemplates.Style.Dramatic) {
        data.motifs.headOption.foreach { firstMotif =>
          val motifName = firstMotif.getClass.getSimpleName
          parts += NarrativeTemplates.tacticsQuestion(motifName, seed, style)
        }
      }
    }
    ...
    if (data.plans.nonEmpty) {
      ...
      val plansHeader = style match {
        case NarrativeTemplates.Style.Coach => "PLANS (Key ideas):"
        case NarrativeTemplates.Style.Dramatic => "THE MASTER PLAN:"
        case _ => "PLANS:"
      }
      ...
      if (style == NarrativeTemplates.Style.Coach || style == NarrativeTemplates.Style.Dramatic) {
         data.plans.headOption.foreach { bestPlan =>
            parts += NarrativeTemplates.rhetoricalQuestion(bestPlan.plan.name, seed, style)
         }
      }
    }
