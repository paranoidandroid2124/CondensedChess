...
object BreakAnalyzer:
  ...
  def analyze(
    ...
    val driver = computePrimaryDriver(breakReady, urgency, advanceOrCapture)
    ...
    PawnPlayAnalysis(
      ...
      notes = s"Break: $breakReady, Passer: $urgency, Tension: $tensionPolicy"
    )
  ...
  private def computeTensionPolicy(
    ...
    if tension == 0 || lockedCenter then
