 ...
 *   sbt "llm/Test/runMain lila.llm.tools.BookCommentaryCorpusRunner"
 *   sbt "llm/Test/runMain lila.llm.tools.BookCommentaryCorpusRunner modules/llm/docs/BookCommentaryCorpus.json modules/llm/docs/BookCommentaryCorpusReport.md"
 */
object BookCommentaryCorpusRunner:
  private val AllowedRepeatedFivegramViolations = 20
  private val LegacyStrategicFallbackEnabled = boolEnv("LLM_LEGACY_STRATEGIC_TEXT_FALLBACK", default = false)
  private val Phase2MaxPvCouplingRatio = doubleEnv("LLM_PHASE2_MAX_PV_COUPLING_RATIO", default = 0.55)
  private val Phase2MinHypothesisProbeHitRate = doubleEnv("LLM_PHASE2_MIN_HYPOTHESIS_PROBE_HIT_RATE", default = 0.60)
  private val Phase2MaxThemeShare = doubleEnv("LLM_PHASE2_MAX_THEME_SHARE", default = 0.35)
  private val Phase2MaxUnknownThemeRate = doubleEnv("LLM_PHASE2_MAX_UNKNOWN_THEME_RATE", default = 0.05)
  ...
  final case class StrategicMetrics(
      ...
      mainPlansCount: Int,
      latentPlansCount: Int,
      absentReasonsCount: Int,
      playableEvidenceBacked: Int,
      playablePvCoupled: Int,
      deferred: Int,
      refuted: Int,
      themeSlots: List[String],
      ...
      probeDepthBudget: Int,
      // Probe decomposition
      probeHitsByPurpose: Map[String, (Int, Int)] = Map.empty,  // purpose -> (hits, total)
      probeMissingSignals: Map[String, Int] = Map.empty          // signal -> miss count
  )
  ...
  final case class Phase2IndependenceGateStatus(
      avgPvCouplingRatio: Double,
      avgHypothesisProbeHitRate: Double,
      themeMaxShare: Double,
      unknownThemeRate: Double,
      maxPvCouplingRatio: Double,
      minHypothesisProbeHitRate: Double,
      maxThemeShare: Double,
      maxUnknownThemeRate: Double,
      strategicCases: Int
  ):
    def passed: Boolean =
      strategicCases > 0 &&
        avgPvCouplingRatio <= maxPvCouplingRatio &&
        avgHypothesisProbeHitRate >= minHypothesisProbeHitRate &&
        themeMaxShare <= maxThemeShare &&
        unknownThemeRate <= maxUnknownThemeRate
  ...
  def main(args: Array[String]): Unit =
    ...
    try
      ...
      val phase2Gate    = phase2IndependenceGate(results)
      val endgameGate   = endgameGateStatus(results, endgameGoldMetrics)
      val strictOk      = advisories == 0 && gate.passed && failedQuality == 0 && endgameGate.passed && phase2Gate.passed
      ...
      if failed == 0 && failedQuality == 0 && (!strictQuality || strictOk) then
        ...
        if !phase2Gate.passed then
          println(
            f"[corpus] ⚠ Phase2 independence gate not met: avgPvCoupling=${phase2Gate.avgPvCouplingRatio}%.3f (target<=${phase2Gate.maxPvCouplingRatio}%.3f), hypothesisProbeHit=${phase2Gate.avgHypothesisProbeHitRate}%.3f (target>=${phase2Gate.minHypothesisProbeHitRate}%.3f), themeMaxShare=${phase2Gate.themeMaxShare}%.3f (target<=${phase2Gate.maxThemeShare}%.3f), unknownThemeRate=${phase2Gate.unknownThemeRate}%.3f (target<=${phase2Gate.maxUnknownThemeRate}%.3f)."
          )
      else
        if failed > 0 then
        ...
        if strictQuality && (advisories > 0 || !gate.passed || !endgameGate.passed || !phase2Gate.passed) then
          System.err.println(
            s"[corpus] ❌ Strict quality mode: non-blocking issues detected (advisories=$advisories, gate=${if gate.passed then "PASS" else "FAIL"}, endgameGate=${if endgameGate.passed then "PASS" else "FAIL"}, phase2Gate=${if phase2Gate.passed then "PASS" else "FAIL"}). Wrote `$outPath`."
          )
        sys.exit(1)
    finally
      ...
      case Some(data) =>
          ...
          try
            ...
            val themeSlots =
              outline.beats
                ...
                .collect { case cid if cid.startsWith("theme_slot:") => cid.stripPrefix("theme_slot:").trim }
                .filter(_.nonEmpty)
                .distinct
        ...
        val semanticConcepts =
          enrichedCtx.semantic
            ...
            .getOrElse(Nil)
        val strategicMetrics = computeStrategicMetrics(
          data = data,
          ctx = enrichedCtx,
          rawProbeResults = c.probeResults.getOrElse(Nil),
          outlineThemeSlots = outlineOpt.map(_.themeSlots).getOrElse(Nil)
        )
  ...
  private def computeStrategicMetrics(
    ...
    val validation = PlanEvidenceEvaluator.validateProbeResults(rawProbeResults, ctx.probeRequests)
    ...
    val evaluated = strategicPartition.evaluated
    val statusNames = evaluated.map(_.status.toString)
    val playableEvidenceBacked =
      statusNames.count(s => s == "PlayableEvidenceBacked" || s == "Playable")
    val playablePvCoupled =
      statusNames.count(_ == "PlayablePvCoupled")
    val deferred =
      statusNames.count(_ == "Deferred")
    val refuted =
      statusNames.count(_ == "Refuted")
    val pvCouplingRatio =
      val playableTotal = playableEvidenceBacked + playablePvCoupled
      if playableTotal <= 0 then 0.0
      else playablePvCoupled.toDouble / playableTotal.toDouble
    ...
    // Probe decomposition: hits/total by purpose, missing signals
    val probeHitsByPurpose =
      hypothesisRequests
        .groupBy(_.purpose.getOrElse("unknown"))
        .view
        .mapValues { reqs =>
          val hits = reqs.count(r => validIds.contains(r.id))
          (hits, reqs.size)
        }
        .toMap
    val probeMissingSignals =
      hypothesisRequests
        .filterNot(r => validIds.contains(r.id))
        .flatMap { req =>
          val result = rawProbeResults.find(_.id == req.id)
          result match
            case Some(pr) =>
              val v = ProbeContractValidator.validateAgainstRequest(req, pr)
              v.missingSignals
            case None => List("no_result_returned")
        }
        .groupBy(identity)
        .view
        .mapValues(_.size)
    ...
    val contractDropRate =
      ...
      else validation.droppedCount.toDouble / rawProbeResults.size.toDouble
    ...
    val fallbackUsed = LegacyStrategicFallbackEnabled && fallbackEligible
    val probeRequestCount = ctx.probeRequests.size
    val probeDepthBudget = ctx.probeRequests.map(_.depth.max(0)).sum
    val strategicThemeSlots =
      strategicPartition.evaluated
        .map(_.themeL1.trim.toLowerCase)
        .filter(_.nonEmpty)
    val themeSlots =
      if strategicThemeSlots.nonEmpty then strategicThemeSlots
      else outlineThemeSlots.map(_.trim.toLowerCase).filter(_.nonEmpty)
    ...
    StrategicMetrics(
      ...
      mainPlansCount = ctx.mainStrategicPlans.size,
      latentPlansCount = ctx.latentPlans.size,
      absentReasonsCount = ctx.whyAbsentFromTopMultiPV.size,
      playableEvidenceBacked = playableEvidenceBacked,
      playablePvCoupled = playablePvCoupled,
      deferred = deferred,
      refuted = refuted,
      themeSlots = themeSlots,
      fallbackEligible = fallbackEligible,
      fallbackUsed = fallbackUsed,
      probeRequestCount = probeRequestCount,
      probeDepthBudget = probeDepthBudget,
      probeHitsByPurpose = probeHitsByPurpose,
      probeMissingSignals = probeMissingSignals
    )
  ...
  private def qualityFindingsFrom(q: QualityMetrics): List[String] =
    List(
      // Hard quality findings should track board-faithful commentary errors, not prose style.
  ...
  private def phase2IndependenceGate(results: List[CaseResult]): Phase2IndependenceGateStatus =
    ...
    val themeSlots =
      strategicList
        .flatMap(_.themeSlots)
        .map(_.trim.toLowerCase)
        .filter(_.nonEmpty)
    val themeCounts = themeSlots.groupBy(identity).view.mapValues(_.size).toMap
    val totalThemes = themeSlots.size
    val themeMaxShare =
      if totalThemes <= 0 then 0.0
      else themeCounts.values.maxOption.getOrElse(0).toDouble / totalThemes.toDouble
    val unknownThemeRate =
      if totalThemes <= 0 then 0.0
      else themeCounts.getOrElse("unknown", 0).toDouble / totalThemes.toDouble
    Phase2IndependenceGateStatus(
      avgPvCouplingRatio = avgPvCouplingRatio,
      avgHypothesisProbeHitRate = avgHypothesisProbeHitRate,
      themeMaxShare = themeMaxShare,
      unknownThemeRate = unknownThemeRate,
      maxPvCouplingRatio = Phase2MaxPvCouplingRatio,
      minHypothesisProbeHitRate = Phase2MinHypothesisProbeHitRate,
      maxThemeShare = Phase2MaxThemeShare,
      maxUnknownThemeRate = Phase2MaxUnknownThemeRate,
      strategicCases = strategicList.size
    )
  ...
  private def renderReport(
    ...
    val avgMainPlansCount = average(strategicList.map(_.mainPlansCount.toDouble))
    val avgLatentPlansCount = average(strategicList.map(_.latentPlansCount.toDouble))
    val avgAbsentReasonsCount = average(strategicList.map(_.absentReasonsCount.toDouble))
    val mainPlansNonEmptyRate = ratio(strategicList.count(_.mainPlansCount > 0), strategicList.size)
    val latentPlansNonEmptyRate = ratio(strategicList.count(_.latentPlansCount > 0), strategicList.size)
    val whyAbsentNonEmptyRate = ratio(strategicList.count(_.absentReasonsCount > 0), strategicList.size)
    val totalPlayableEvidenceBacked = strategicList.map(_.playableEvidenceBacked).sum
    val totalPlayablePvCoupled = strategicList.map(_.playablePvCoupled).sum
    val totalDeferred = strategicList.map(_.deferred).sum
    val totalRefuted = strategicList.map(_.refuted).sum
    val totalEvidenceStates = totalPlayableEvidenceBacked + totalPlayablePvCoupled + totalDeferred + totalRefuted
    val themeSlotsAll = strategicList.flatMap(_.themeSlots)
    val themeSlotUsageCases = strategicList.count(_.themeSlots.nonEmpty)
    val themeSlotUsageRate = ratio(themeSlotUsageCases, strategicList.size)
    val themeSlotDistribution = themeSlotsAll.groupBy(identity).view.mapValues(_.size).toList.sortBy((_, n) => -n)
    val themeEntropy = normalizedEntropy(themeSlotDistribution.map(_._2))
    val fallbackEligibleCases = strategicList.count(_.fallbackEligible)
    val fallbackUsedCases = strategicList.count(_.fallbackUsed)
    val fallbackEligibleRate = ratio(fallbackEligibleCases, strategicList.size)
    val fallbackUsedRate = ratio(fallbackUsedCases, strategicList.size)
    val avgProbeRequestCount = average(strategicList.map(_.probeRequestCount.toDouble))
    val probeRequestP95 = percentileInt(strategicList.map(_.probeRequestCount), 0.95)
    val avgProbeDepthBudget = average(strategicList.map(_.probeDepthBudget.toDouble))
    val probeDepthBudgetP95 = percentileInt(strategicList.map(_.probeDepthBudget), 0.95)
    ...
    if qualityList.nonEmpty then
      ...
      sb.append("\n")
      sb.append("### Plan-First Transition Metrics\n\n")
      sb.append(s"- Legacy fallback env enabled: $LegacyStrategicFallbackEnabled\n")
      sb.append(
        f"- mainStrategicPlans: avgCount=$avgMainPlansCount%.3f, nonEmptyRate=$mainPlansNonEmptyRate%.3f\n"
      )
      sb.append(
        f"- latentPlans: avgCount=$avgLatentPlansCount%.3f, nonEmptyRate=$latentPlansNonEmptyRate%.3f\n"
      )
      sb.append(
        f"- whyAbsentFromTopMultiPV: avgCount=$avgAbsentReasonsCount%.3f, nonEmptyRate=$whyAbsentNonEmptyRate%.3f\n"
      )
      if totalEvidenceStates > 0 then
        sb.append(
          f"- PlanEvidenceState distribution: PlayableEvidenceBacked=$totalPlayableEvidenceBacked (${totalPlayableEvidenceBacked.toDouble / totalEvidenceStates}%.3f), PlayablePvCoupled=$totalPlayablePvCoupled (${totalPlayablePvCoupled.toDouble / totalEvidenceStates}%.3f), Deferred=$totalDeferred (${totalDeferred.toDouble / totalEvidenceStates}%.3f), Refuted=$totalRefuted (${totalRefuted.toDouble / totalEvidenceStates}%.3f)\n"
        )
      else sb.append("- PlanEvidenceState distribution: no evaluated hypotheses\n")
      sb.append(
        f"- theme_slot usage: casesWithThemeSlot=$themeSlotUsageCases/${strategicList.size} (${themeSlotUsageRate}%.3f)\n"
      )
      if themeSlotDistribution.nonEmpty then
        sb.append("- theme_slot distribution: ")
        sb.append(themeSlotDistribution.take(12).map { case (theme, n) => s"$theme=$n" }.mkString(", "))
        sb.append("\n")
      else sb.append("- theme_slot distribution: none\n")
      sb.append(
        f"- Legacy fallback usage: eligibleCases=$fallbackEligibleCases/${strategicList.size} (${fallbackEligibleRate}%.3f), usedCases=$fallbackUsedCases/${strategicList.size} (${fallbackUsedRate}%.3f)\n"
      )
      sb.append(
        f"- Probe budget: avgRequests=$avgProbeRequestCount%.3f, p95Requests=$probeRequestP95%d, avgDepthBudget=$avgProbeDepthBudget%.3f, p95DepthBudget=$probeDepthBudgetP95%d\n"
      )
      // Probe Hit Decomposition (aggregate)
      val aggHitsByPurpose = scala.collection.mutable.Map.empty[String, (Int, Int)]
      val aggMissingSignals = scala.collection.mutable.Map.empty[String, Int]
      strategicList.foreach { s =>
        s.probeHitsByPurpose.foreach { case (purpose, (hits, total)) =>
          val (prevH, prevT) = aggHitsByPurpose.getOrElse(purpose, (0, 0))
          aggHitsByPurpose.update(purpose, (prevH + hits, prevT + total))
        }
        s.probeMissingSignals.foreach { case (signal, count) =>
          aggMissingSignals.update(signal, aggMissingSignals.getOrElse(signal, 0) + count)
        }
      }
      if aggHitsByPurpose.nonEmpty then
        sb.append("- Probe hit decomposition by purpose:\n")
        aggHitsByPurpose.toList.sortBy((_, v) => -v._2).foreach { case (purpose, (hits, total)) =>
          val rate = if total == 0 then 1.0 else hits.toDouble / total.toDouble
          sb.append(f"  - $purpose: $hits/$total (${rate}%.3f)\n")
        }
      if aggMissingSignals.nonEmpty then
        sb.append("- Most frequent missing signals:\n")
        aggMissingSignals.toList.sortBy(-_._2).take(6).foreach { case (signal, count) =>
          sb.append(s"  - $signal: $count\n")
        }
      sb.append(
      ...
      sb.append(
        f"- Phase 2 independence gate: ${if phase2Gate.passed then "PASS" else "FAIL"} (avgPvCoupling=${phase2Gate.avgPvCouplingRatio}%.3f target<=${phase2Gate.maxPvCouplingRatio}%.3f; hypothesisProbeHit=${phase2Gate.avgHypothesisProbeHitRate}%.3f target>=${phase2Gate.minHypothesisProbeHitRate}%.3f; themeMaxShare=${phase2Gate.themeMaxShare}%.3f target<=${phase2Gate.maxThemeShare}%.3f; unknownThemeRate=${phase2Gate.unknownThemeRate}%.3f target<=${phase2Gate.maxUnknownThemeRate}%.3f)\n"
      )
      sb.append(f"- Theme entropy (normalized): $themeEntropy%.3f\n")
      sb.append(
    ...
    results.foreach { r =>
      ...
      r.strategic.foreach { s =>
        ...
        sb.append(
          s"- Strategic partition: main=${s.mainPlansCount}, latent=${s.latentPlansCount}, whyAbsent=${s.absentReasonsCount}, states=[PlayableEvidenceBacked=${s.playableEvidenceBacked}, PlayablePvCoupled=${s.playablePvCoupled}, Deferred=${s.deferred}, Refuted=${s.refuted}], fallbackEligible=${s.fallbackEligible}, fallbackUsed=${s.fallbackUsed}, probeRequests=${s.probeRequestCount}, probeDepthBudget=${s.probeDepthBudget}\n"
        )
        if s.themeSlots.nonEmpty then
          sb.append(s"- Theme slots: ${s.themeSlots.mkString(", ")}\n")
        if s.probeHitsByPurpose.nonEmpty then
          val detail = s.probeHitsByPurpose.toList.sortBy(-_._2._2).map { case (p, (h, t)) => s"$p=$h/$t" }.mkString(", ")
          sb.append(s"- Probe hits: $detail\n")
        if s.probeMissingSignals.nonEmpty then
          val detail = s.probeMissingSignals.toList.sortBy(-_._2).map { case (sig, n) => s"$sig=$n" }.mkString(", ")
          sb.append(s"- Missing signals: $detail\n")
      }
      ...
      r.outline.foreach { o =>
        ...
        if o.themeSlots.nonEmpty then sb.append(s"- Outline theme slots: ${o.themeSlots.mkString(", ")}\n")
  ...
  private def percentileInt(values: List[Int], p: Double): Int =
    if values.isEmpty then 0
    else
      val sorted = values.sorted
      val idx = math.ceil(p * sorted.size.toDouble).toInt - 1
      sorted(idx.max(0).min(sorted.size - 1))

  private def normalizedEntropy(counts: List[Int]): Double =
    val total = counts.filter(_ > 0).sum.toDouble
    if total <= 0 then 0.0
    else
      val probs = counts.filter(_ > 0).map(_.toDouble / total)
      val raw = -probs.map(p => p * (math.log(p) / math.log(2.0))).sum
      val support = probs.size
      if support <= 1 then 0.0
      else (raw / (math.log(support.toDouble) / math.log(2.0))).max(0.0).min(1.0)
  ...
  private def doubleEnv(name: String, default: Double): Double =
    sys.env
      .get(name)
      .flatMap(v => scala.util.Try(v.trim.toDouble).toOption)
      .getOrElse(default)
