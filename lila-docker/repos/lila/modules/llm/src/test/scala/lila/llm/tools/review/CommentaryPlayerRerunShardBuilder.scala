...
object CommentaryPlayerRerunShardBuilder:
  ...
  import lila.llm.tools.realpgn.RealPgnNarrativeEvalRunner.{ Corpus, CorpusGame }
  ...
  private def renderRealPgnScript(shards: List[ShardMeta]): String =
    renderScript(shards, "run shard real-PGN signoff") { shard =>
      ...
      val rawDir = s"${shard.runDir}\\raw"
        s"""sbt 'llm/test:runMain lila.llm.tools.realpgn.RealPgnNarrativeEvalRunner ${quoteSbtArg(shard.corpusPath)} ${quoteSbtArg(reportMd)} ${quoteSbtArg(reportJson)} ${quoteSbtArg(rawDir)} --depth 10 --multi-pv 3'"""
    }
  ...
  private def renderBookmakerScript(shards: List[ShardMeta]): String =
    renderScript(shards, "run shard bookmaker outputs") { shard =>
      ...
      val rawDir = s"${shard.runDir}\\bookmaker_raw"
        s"""sbt 'llm/test:runMain lila.llm.tools.bookmaker.BookmakerCorpusRunner ${quoteSbtArg(shard.manifestPath)} ${quoteSbtArg(outPath)} ${quoteSbtArg(rawDir)} --depth 10 --multi-pv 3'"""
    }
  ...
  private def renderReviewQueueScript(shards: List[ShardMeta]): String =
    renderScript(shards, "build shard review queue") { shard =>
      ...
      val summaryPath = s"${shard.runDir}\\review_queue_summary.json"
        s"""sbt 'llm/test:runMain lila.llm.tools.review.CommentaryPlayerReviewQueueBuilder ${quoteSbtArg(shard.manifestPath)} ${quoteSbtArg(bookmakerPath)} ${quoteSbtArg(chronicleReport)} ${quoteSbtArg(queuePath)} ${quoteSbtArg(summaryPath)}'"""
    }
...
try {
      sbt 'llm/test:runMain lila.llm.tools.realpgn.RealPgnNarrativeEvalReportMerge ${quoteSbtArg(reportRoot)} ${quoteSbtArg(mergedMd)} ${quoteSbtArg(mergedJson)}'
} finally {
  Pop-Location
}
"""
