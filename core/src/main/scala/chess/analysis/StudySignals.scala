...
object StudySignals:
  ...
  private def computeStudySignals(p: PlyOutput, opening: Option[Opening.AtPly]): (Double, List[String]) =
    ...
    // Rely on p.phaseLabel computed by ConceptLabeler
    val phaseShift = if p.phaseLabel.nonEmpty then 12.0 else 0.0
    ...
    val phaseN   = clamp(phaseShift / 15.0, 0.0, 1.0)
    ...
    if theoryFork then tags += "OpeningTheoryBranch"
    p.phaseLabel.foreach(tags += _)
    if planShift >= 0.3 then tags += "PlanChange"
