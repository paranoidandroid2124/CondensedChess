...
object page:
  ...
  // Activity page simplified - activity module deleted
  def activity(
      activities: Seq[Any],  // activity module deleted
      info: UserInfo,
      social: UserInfo.Social
  )(using Context) =
    val u = info.user
    Page(s"${u.username} : Activity")  // simplified title
      .graph(
        OpenGraph(
        ...
        main(cls := "page-menu", ui.dataUsername := u.username)(
          ...
          div(cls := "page-menu__content box user-show")(
            ...
            div(cls := "angle-content")(
              // activity view removed - module deleted
              p("Activity view not available")
            )
          )
        )
