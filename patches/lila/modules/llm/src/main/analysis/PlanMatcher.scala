...
object PlanMatcher:
  ...
  private def scoreKingsideAttack(
    ...
    val relevantMotifs = motifs.filter:
      ...
      case m: CheckMotif    => m.targetSquare.file.isKingside
      case m: PieceLift     => true
      case m: ForkMotif     => m.targets.contains(King)
      case _                => false
  ...
  private def scorePerpetualCheck(
    ...
    val checks = motifs.collect { case m: CheckMotif => m }
  ...
  private def scoreDefensiveConsolidation(
    ...
    val defensiveMotifs = motifs.filter:
      ...
      case m: CaptureMotif if m.captureType == CaptureType.Exchange => true
      case _ => false
